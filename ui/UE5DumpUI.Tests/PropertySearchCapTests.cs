...
public class PropertySearchCapTests
{
    ...
    public void TheClampRangeIsTheSameNumberInTheUiTheControlAndTheDll()
    {
        ...
        Assert.Equal(50000, Constants.MaxPropertySearchCap);
        ...
        Assert.Equal(Constants.MaxPropertySearchCap.ToString(), nud.Groups["max"].Value);
    ...
    public void TheDllClampsTheSameCeilingBecauseTheUiClampIsOnlyAConvenience()
    {
        ...
        Assert.Contains($"if (limit > {Constants.MaxPropertySearchCap}) limit = {Constants.MaxPropertySearchCap};", handler);
    ...
    public void TheCapAdviceOffersRaiseMaxOnlyWhileRaisingItIsPossible()
    {
        ...
        static string Advice(int cap, bool gameOnly)
        {
            ...
            if (cap < Constants.MaxPropertySearchCap) a += $", or raise Max above {cap:N0}";
            return a;
        }

        Assert.Contains("raise Max above 200", Advice(200, gameOnly: true));
        Assert.Contains("raise Max above 49,999", Advice(Constants.MaxPropertySearchCap - 1, true));
        Assert.DoesNotContain("raise Max", Advice(Constants.MaxPropertySearchCap, gameOnly: true));
