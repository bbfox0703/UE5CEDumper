...
| any | **CasePreservingName** (= `WITH_EDITORONLY_DATA`, so 0 in every packaged build): `sizeof(FName)` grows 0x8 → **0xC** (adds DisplayIndex; three int32, alignof 4, no pad), while the UObject `NamePrivate`→`OuterPrivate` **slot** grows to **0x10** (OuterPrivate is 8-aligned). FField::Flags moves only **+4** (NamePrivate is 0xC at 0x28, so Flags 0x30 → 0x34, and FField stays 0x38); FProperty's head (ArrayDim .. Offset_Internal) does **not** move, and only RepNotifyFunc's tail moves +8 (RE-UE4SS 4_27_CasePreserving vs 4_27, `[VND583-DOC UEP-D2]`). ⚠ 0x10 is the slot, **not** the size — see the A9 note on `DynOff::bCasePreservingName`. Must use `DynOff` dynamic detection |
