# Dev Log — archived (builds 2220–2747)

Split out of [`../dev-log.md`](../dev-log.md) on 2026-08-25, when the live file reached
**680 KB** — well past the 400 KB the earlier splits used as their trigger. **Nothing was
edited, only moved**, so every entry reads exactly as it did in the live log.

The live log keeps **builds 2779+**. Older halves, newest range first:
...
⚠ The boundary is a DATE (2026-08-10), so the build ranges either side are contiguous but
not perfectly ordered within a day — grep the build number, not the position.
