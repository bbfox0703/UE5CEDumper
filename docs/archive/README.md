...
| `dev-log-2026-08-pre-build-3263.md` | Builds 2779–3261 of the running milestone log (2026-08-10 … 2026-08-17), 75 entries | The fourth split, at **457 KB / 6,868 lines** (the 400 KB trigger again). Cut on the **date** boundary 2026-08-18; 46 of the 75 entries are the single day 2026-08-17 (audit #5's fix run). **Nothing was edited, only moved**, asserted in the split script before anything was written (prefix + moved + suffix == the original) and re-checked line by line afterwards against `HEAD`. The live log keeps builds 3263+ at 176 KB; its header and its `## Older entries` pointer name this file. `check_audit_register` reads only the live log, so the AA/AB/AE fixes of 2026-08-15/17 no longer count toward its "dev-log claims" line; their ✅ rows in the audit doc are unaffected. |
