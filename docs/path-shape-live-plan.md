...
|---|---|---|---|
| 1 | `[PATH-MODULE-NAME-UTF8]` | `module_name` is the real UTF-8 name, for all five | ⏳ |
| 2 | `[PATH-CE-MODULE-VIEW]` (DLL side) | `ce_base` names the module as CE does: `DumperTest51?-…` for ™, `夫-…` for `功夫-…` (the 0x5C cut) | ⏳ |
...
| 16 | `[PROXY-RISKNOTE-WIPED]` | A deploy's import-risk note is still on the row after the refresh (Deploy and Update All) | ⏳ |
