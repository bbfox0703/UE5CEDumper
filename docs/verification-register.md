...
| **SW5** | **`PropertyXrefDialog`'s push-result branches** (`c55f2237`). | ⛔⛔ **THE RED ARM IS NOT REACHABLE AS WRITTEN — verified in source 2026-09-09.** The row says "with Cheat Engine closed, 'Push to CE disassembler' must produce the RED …". It cannot: `PropertyXrefDialog.cs:387` sets `_btnDisasm.IsEnabled = has && `SharedAobMaker?.IsAvailable == true`, and the handler re-reads the same cached flag at `:471` — `if (bridge == null || !bridge.IsAvailable) return;` — returning **silently, with no status text at all**. With CE closed for the whole session the button is dead and a click prints nothing. Reaching red needs CE up long enough to set `IsAvailable` (the flag is refreshed only by user-triggered probes, never a timer), a row selected, then CE killed while the MODAL dialog holds focus so no activation probe can clear the flag, then click. Rewrite the acceptance before running it. **UI side:** the amber half-success branch needs CE to accept the record and refuse the navigation. ⚠ **Reachable with the REAL plugin, corrected 2026-09-09** — both handlers go through CE's overwritable Lua global `synchronize`, so a wrapper that errors only for the DisassemblerView caller splits them deterministically. That yields real-CE evidence instead of a stand-in server's transcript. Before the fix all three produced the same green label. |
