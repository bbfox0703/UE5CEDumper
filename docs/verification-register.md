...
| **SW4** | **The CE-side delegate pad** — `delegate_pad` on the wire → `CeOffset()` at 21 emit sites, and `ue5_dissect.lua` taking the delegate width off the wire (`[D4B-TAIL]`, `[STALE-NONE]`). | Unit-pinned (`CeXmlDelegatePadTests`, 4 cases, mutation-tested; `dissect_test.lua`, 10 checks). ⚠ **Never pasted into Cheat Engine.** **CE side:** on a **checked build** (Development/Debug — the pad is 0 in Shipping, so a Shipping test proves nothing here) a delegate row's group must resolve to the InvocationList, not to address 0. **DLL side:** the same field's `delegate_pad` must read 8 in the pipe payload. ⭐⛔ **THE DLL SIDE WAS BROKEN, and this row's own wording is what says so** — found 2026-09-09 by `sw7_stale_arm.py`, fixed the same day (`Fern.cpp`). The emission sat inside the ARRAY branch (`if (arrayCount >= 0)` → `if (!arrayInnerType...)`), which a scalar `DelegateProperty` never enters: `arrayCount` stays `-1`. So `Ubel.cpp:5507` set the pad on every checked build and the value **never left the process**, while `Ubel.cpp:6004` (MulticastInline) reports an invocation list, cleared the gate, and was never affected — the loss was exactly one field KIND. Live effect: `CeXmlExportService.CeOffset` added 0 for a scalar DelegateProperty, i.e. **the address-0 CE record this very row was written to catch**. ⚠ `CeXmlDelegatePadTests` (4 cases, mutation-tested) stayed GREEN throughout, because it builds `LiveFieldValue` in C# with `DelegatePad` already set and never crosses DLL → wire → C#. ⭐ That is this register's thesis in one defect: unit-green on both sides, broken in the gap between them. **DLL side now MEASURED** on the wire (DumperTest 5.4 Development): `Del_Unicast` carries `delegate_pad: 8` with no `count` key at all, `Multicast_Inline` carries `count: 1` + `array_elem_size: 16` + `delegate_pad: 8`; on Shipping both are absent (pad 0, emitted only when non-zero). **The CE side remains open** — still never pasted into Cheat Engine. ⚠ **Two shape corrections from the 2026-09-09 scouting pass, to check before building that arm:** a **scalar** `DelegateProperty` never emits `<Offsets>` at all — `MapCeField` routes it to a flat `"8 Bytes", ShowAsHex` leaf — so "the group must resolve to the InvocationList" describes the MULTICAST shape, not the one `[SW7]`'s wire fix was about; and the UI produces **two different addresses for the same field**, `CeOffset = Offset + DelegatePad` for the clipboard XML but an UNPADDED `FieldAddress` elsewhere. Confirm which one the CE record actually carries before asserting on it. |
