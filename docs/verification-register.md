...
| **SW7** | **The `(stale)` narrowing's stale ARM** (`0b8f89e5`). | The unbound half was verified live (`Arr_Delegates[0]` went `(stale)::None` → `(unbound)` on DumperTest, both configurations). ⚠ **The other half was not**: a delegate bound to an object that is then garbage-collected must still render `(stale)::Func`. **DLL side:** `ResolveWeakObjectPtr` returns 0 while the FName still resolves. **UI side:** the row says `(stale)::` and not `(unbound)`. Needs a fixture that destroys a bound target and forces GC — `Spawn_*` + `ForceGarbageCollection` already exist on `DumperTestActor`. ⛔ **A rig must NOT locate the element via `delegate_pad`**: it is set at exactly two scalar sites (`Ubel.cpp:5507`, `:6004`) and `Fern.cpp:1501` emits it only when non-zero, so it is **absent for a TArray of delegates**. Deriving the element address from it lands in the access detector, the walk returns the healthy string, and the run passes while measuring nothing. Derive from `array_elem_size`, as `d4b_delegate_pad.py` does. |
