...
| **SW3** | **CE Lua untick, 3 sites** (`b344d63d`) + **Invoke's `onUnreadable`** (`a55ef2b5`). | Pinned by **gate 17a** (source text) and by assertions over the GENERATED Lua that no interpreter runs — except site (c), which `invoke_helper_test.lua` does drive. ⛔ **SITE (c) IS A DOCUMENTATION SAMPLE, NOT EXECUTABLE LUA — verified by hand 2026-09-09.** `scripts/ue5_invoke_helper.lua` opens `--[[` on line 1 and does not close until line 67, so the untick at `:48` sits INSIDE a block comment. ⚠ **This is NOT a gate defect** — `check_ce_untick_placement.py:105` says "Block comments INCLUDED" and its header (:23-24) gives the reason: these copy-me samples themselves taught the wrong pattern for five weeks after the C# generators were fixed, so scanning them is the point. What it means for THIS row is narrower and must not be overstated: that site cannot be exercised by running anything, because there is nothing to run. Scope the CE arm to the sites that are real code, and say which ones those are. **CE side:** with the game killed mid-invoke, the record must UNTICK and the Lua Engine window must report a dead process, not a busy mailbox. **DLL side:** the mailbox `status` field must say which failure it was; the script must not be guessing. |
