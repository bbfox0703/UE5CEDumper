...
|---|---|---|
| **SW1** | **D2** — a scan worker that THROWS must not let the run report complete (`e6360903`, `workerFaulted` / `incomplete()`). | ⚠ **"No fixture exists" was WRONG — corrected 2026-09-09.** A feasibility pass found the route: the DLL is built `/EHa` (`dll/CMakeLists.txt:155,186`), so the `catch (...)` at `Aura.cpp:277` catches a hardware **access violation** as well as a C++ throw — and `UE5_SetObjectDecryption` is an exported C ABI setter (`Frieren.h:81`) for a raw indirect call that every `GetByIndex` makes inside the worker. Point it at `0x1000`, scan, pass NULL to restore identity: manufacture-and-restore, no DLL edit. ⛔ **The all-workers variant is NOT enough** — it yields `deadline_hit` over an EMPTY set, which cannot show a set is *partial*. Arm it MID-SCAN on a large title, or use the tid-gated fault-injection build. ⚠ While armed, any other caller of `DecryptObjectPtr` outside the worker try has no handler and kills the game: nothing else on the wire. **DLL side:** a `SCAN`-category log line naming the faulted worker. **UI side:** the partial-result notice must appear. ⚠ Its *deliberate residual* is that a fault reports through `deadline_hit`, so the UI says "DEADLINE HIT" for a cause that was not a deadline — a live run must confirm the claim is at least TRUE (the set really is partial) even while the cause is mislabelled. |
