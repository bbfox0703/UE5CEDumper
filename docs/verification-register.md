...
|---|---|---|
| **SW1** | 🟡 **HALF-CLOSED 2026-09-09 `[SW1-WORKERFAULT-2026-09-09]`** — D2, a scan worker that THROWS must not let the run report complete (`e6360903`). | ⭐ **The stated property is PROVEN**, `tools/verify/sw1_worker_fault.py --serial`, on **EVERSPACE 2** (1,137,983 objects, save loaded). The DLL is `/EHa`, so the `catch (...)` at `Aura.cpp:278` catches a hardware **access violation**; `UE5_SetObjectDecryption` is an exported C ABI setter for the raw indirect call every `GetByIndex` makes inside the worker. Pointed at `0x1000` mid-scan, restored to NULL after: control `total=16777 scanned_objects=1137983 deadline_hit=False` → armed `deadline_hit=True`, and `offsets-0.log` carried `[ERROR] [OARR] ParallelGObjectsScan: worker tid=0 [0,1154906) faulted — that index range was NOT walked; results are partial` — **a line never emitted before in this repo's history**. The run did not report a complete result, which is what D2 says. ⛔ **WHAT IS NOT PROVEN, and is measured to be otherwise: that the result is PARTIAL. It is EMPTY** (`total=0 scanned_objects=0`). `parallel=false` is ONE chunk over the whole array, so the unwind discards the entire walk. The register warned the all-workers variant "yields deadline_hit over an EMPTY set"; the SERIAL form does too, for the same structural reason, even with the arm landing mid-scan. ⭐ **The log line has since been corrected** (2026-09-09): it used to end "results are partial", a claim about the whole scan that one worker cannot make and that this run showed to be false, and now reads "…that index range was NOT walked. Anything this run reports is missing at least that range, and is EMPTY if this was the only chunk" — verified live. ⛔ **And the PARALLEL form could not be made to fault at all**: armed across the ENTIRE run (0.0 s in, 900 ms hold vs a 458 ms scan) the response was byte-identical to the control. Across four armed runs in one process the log holds **4 SET/CLEARED pairs and 1 fault line** — only the first faulted, which is CONSISTENT WITH (not proof of) the scan index being built once and reused, so later runs never reach `DecryptObjectPtr`. **What is still owed:** one armed run per FRESH process, which is what would show a genuinely partial set. ⚠ **THREE REGISTER CLAIMS CORRECTED.** (a) The fault line is **OARR → `offsets-*.log`**, not "a SCAN-category log line" — "SCAN" is a real category mapping to scan.log, so the old wording sends an operator to a file that can never contain it. (b) "Any other caller of `DecryptObjectPtr` outside the worker try has no handler and kills the game" is **largely false**: `Fern::DispatchCommand` (`Fern.cpp:6425`), `RunThreadGuarded`, `Mimic.cpp:358`, `Stark.cpp:205` and `Schlacht.cpp:561` all catch it under `/EHa`; only the raw C ABI exports are unguarded. (c) `deadline_hit` has **three** non-fault causes — a real deadline, a cancel, and silently a `max_results` CAP HIT (`Aura.cpp:8244`) — so the control must come back `deadline_hit=False` AND `total < max_results` or the run is void. The rig asserts both. |
...
| **SW3** | 🟡 **HALF-CLOSED 2026-09-09 `[SW3-UNTICK-2026-09-09]`** — CE Lua untick + Invoke's `onUnreadable` (`b344d63d`, `a55ef2b5`). | **The `onUnreadable` arm is PROVEN, live, in real Cheat Engine.** Setup: DumperTest 5.4 Development injected, CE attached, the UI's `INV` button pushed a generated AA record (`Invoke: DumperTestActor::V8_RemoveOneTableRow`) straight into CE's table via the AOBMaker plugin — so the artefact under test is `InvokeScriptGenerator`'s real output, not a hand-written stand-in. The game was then KILLED while CE kept the process open, and the record ticked. **CE side, both halves:** the Lua reported
