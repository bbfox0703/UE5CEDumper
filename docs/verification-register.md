...
> 📦 **4 more closed sections (473 lines) were archived
> 2026-09-26** to [archive/verification-register-closed-2026-09-26-build-3559.md](archive/verification-register-closed-2026-09-26-build-3559.md) — moved verbatim, nothing
> rewritten: `[GWORLDACTORCHAIN-2026-08-26]`, `[A6-BOOLFIELD-2026-09-05]`, the AA(B) / FIRE past-cap row
> (audit #5 X2; its two rig traps are in working-lessons.md now) and `[W23-PIPE-2026-08-17]`. ⭐ A `✅` was
> again **not** sufficient: every `✅` section or row whose body still carries a still-owed line, an
> unexercised leg of its own acceptance, an unexplained observation, an unfiled lead or a standing
> warning found nowhere else was **kept here**. No `⬜` / `🟡` heading moved. See that file's header.
...
2. **Do not open a row for A5's tie-break or its new warning.** `PreferStride` changes no outcome in practice over `{16, 24, 32, 20, 40}` — ⚠ **but NOT for the reason first recorded here, corrected 2026-09-06.** The old wording was *"a provable no-op — every divisor pair already has the smaller candidate earlier in the list"*. The divisor halves are true (`16`@0 before `32`@2, `20`@3 before `40`@4), but `PreferStride` fires on **any** equal score, not only on divisor ties (`Lineal.h`: `score == bestScore && bestStride != 0 && stride < bestStride`), and the list holds two **non-divisor** pairs whose smaller member sits **later** — `(24, 20)` and `(32, 20)`. On a tie in either, old and new select differently. The conclusion survives on a different argument: 24/20 and 32/20 are neither multiples nor divisors, so on a correct pool one of each pair lands off-item and loses on score long before a tie — which is what `Lineal.h`'s own tie comment actually says. **The decision not to open a row stands; do not cite the old sentence as a proof.**, and the high-null warning is an anomaly detector whose silence on a healthy pool is the expected result. Both belong in the checklist's §1/§3, not the register.
...
### ⭐ Feasibility, measured 2026-09-09 — **none of these nine is blocked**

A 12-agent pass (nine assessors, three adversarial lenses) went looking for the resource each row needs. Every row came back runnable on this machine; the corrections that pass produced are folded into the rows above, each marked *corrected 2026-09-09*.

⭐ **CE does not need a human.** `D:\Github\AOBMaker\tools\ce-verify-harness\` is the precedent: CE runs every `.lua` in `<CheatEngineDir>\autorun\custom\` at startup, and Lua can `openProcess()`, write files and `closeCE()`. Two shapes exist there — a self-terminating batch (`run.py`) and hold-open-while-an-outside-process-drives-the-pipe (`plugin_probe.py`, `aobmaker-plugin-hold.lua`). Both are inert unless a job file sits beside them. ⚠ Measured here: `C:\Program Files\Cheat Engine\autorun\custom` **is writable**, and `plugin_probe.py` drives `\\.\pipe\AOBMakerCEBridge` — the SAME pipe `AobMakerBridgeService.cs:18` connects to. So SW3/SW4/SW5 need CE, not a person.
...
|---|---|---|
| **SW1** | ✅ **CLOSED 2026-09-09 `[SW1-WORKERFAULT-2026-09-09]`** — D2, a scan worker that THROWS must not let the run report complete (`e6360903`). | **PASS with a genuinely PARTIAL result**, `tools/verify/sw1_worker_fault.py`, EVERSPACE 2. The DLL is `/EHa`, so the `catch (...)` at `Aura.cpp:278` catches a hardware **access violation**; `UE5_SetObjectDecryption` is an exported C ABI setter for the raw indirect call every `GetByIndex` makes inside the worker. Pointed at `0x1000` mid-scan, restored to NULL after — manufacture-and-restore, no DLL edit.
...
&nbsp;&nbsp;`offsets-0.log: worker tid=5 [24950,29940) faulted … / worker tid=13 [64870,69860) faulted …`
`total` is **1649 — greater than zero and smaller than the control's 4115**: some workers faulted and others finished, which is what makes the set PARTIAL rather than empty, the distinction the register said the all-workers variant could never show. ⛔ **THE ARMED SCAN MUST BE THE PROCESS'S FIRST** — `--control-from` exists for that. Measured: a first parallel scan runs 656 ms and arming inside it faults workers; a second scan in the same process runs 303 ms, and four armed runs across it produced **4 SET/CLEARED pairs and not one fault**, even armed across the ENTIRE scan. Something a first scan establishes is reused rather than re-walked. ⚠ An OBSERVATION, not a mechanism — `Aura.h:1741`'s "ScanForValue's index builder" fits, but a `--serial` second scan on DumperTest DID fault, so the forms differ and nothing here says why. ⚠ **The SERIAL form cannot produce a partial result at all**, structurally: one chunk over the whole array, so the unwind discards everything (`total=0 scanned_objects=0` on a save-loaded ES2 even with a mid-scan arm). `--serial` is kept because measuring that is what proves the parallel form is the one that shows PARTIAL. ⚠ **THREE REGISTER CLAIMS CORRECTED.** (a) The fault line is **OARR → `offsets-*.log`**, not "a SCAN-category log line" — "SCAN" is a real category mapping to scan.log, so the old wording sent an operator to a file that can never contain it. (b) "Any other caller of `DecryptObjectPtr` outside the worker try has no handler and kills the game" is **largely false**: `Fern::DispatchCommand` (`Fern.cpp:6425`), `RunThreadGuarded`, `Mimic.cpp:358`, `Stark.cpp:205` and `Schlacht.cpp:561` all catch it under `/EHa`; only the raw C ABI exports are unguarded. (c) `deadline_hit` has **three** non-fault causes — a real deadline, a cancel, and silently a `max_results` CAP HIT (`Aura.cpp:8244`) — so the control must come back `deadline_hit=False` AND `total < max_results` or the run is void. The rig asserts both. ⭐ The worker's log line was also **corrected** as part of this: it used to end "results are partial", a claim about the whole scan that one worker cannot make and that the serial run showed to be false. |
| **SW2** | ✅ **CLOSED 2026-09-09 `[SW2-CLIPDELIVERY-2026-09-09]`** — the clipboard delivery family (`89887dee`, `7de16071`, `cb80ae95`). | **PASS**, `tools/verify/sw2_clipboard_delivery.py` (`prepare` → one click → `verify`), against the AOT-trimmed `dist\UE5DumpUI.exe` (57,443,328 B, sha `3e36d227`), clipboard held by `clipboard_hold.py`, CE closed. ⭐ **Both halves are FILE-observable**, which is what makes this two-sided rather than a screenshot — `Logs\UE5DumpUI\view-0.log` carried exactly two lines in the run window, 1 ms apart: `15:43:01.168 [WARN] Clipboard copy FAILED — nothing was copied, the app is unaffected. System.Runtime.InteropServices.COMException: ...` (the PLATFORM branch, logged immediately before its `return false`, so it IS `CopyToClipboardAsync` having returned false) and `15:43:01.169 [WARN] CE inject bootstrap (dll=...) reached neither CE nor the clipboard` (the SITE, on the statement right after the one assigning the status text). **UI side, rendered:** *"ERROR: could not write to the clipboard — the inject bootstrap was NOT delivered. The clipboard still holds whatever was there before, so do not paste. Another application may be holding it open; try again."* **Unit half:** `ClipboardDeliveryTests` (incl. `RefusingPlatform`) 6/6 and `ClipboardWriteGuardTests` 12/12, run via `dotnet test` — ⛔ never `-Target Test`, which republishes `dist\` NON-TRIMMED; `dist` was SHA-checked unchanged afterwards. ⭐⭐ **THE WITNESS THE APP CANNOT FAKE:** a sentinel string was written to the clipboard BEFORE the lock and was still there afterwards — so "nothing was copied" is corroborated from outside the process, not just asserted by it. ⛔ **TRAPS THE RIG ASSERTS.** (a) **The platform line PREDATES the fix** — it shipped with `[PASTECRASH-2026-08-18]`, before `ClipboardDelivery` existed, so an older exe writes it too and the run would be green over a binary without the behaviour under test (the SW8 stale-proxy shape). `prepare` byte-greps the exe for strings the FIX introduced (`reached neither CE nor the clipboard` ×4, `so do not paste` ×1, `was NOT delivered` ×2). (b) **Failure without a platform call**: `ClipboardDelivery.cs:62` returns false on a null platform or an EMPTY PAYLOAD *without calling the platform*, producing the identical status text and site warn — so the platform line is required, not optional. (c) **The wrong false branch**: `CopyGuardedAsync` has a second `return false` logging "Clipboard copy did nothing: no main window / no clipboard"; the exact prefix is pinned AND a `COMException` is required. (d) **The single-instance mutex** would send the clicks to any already-running UI — possibly a ~107 MB non-trimmed one — which writes to the SAME log; the running image is resolved and SHA-matched against `dist`. (e) **CE running makes the branch unreachable** (`if (!sentToCe)`), so `prepare` refuses. (f) **Hold expiry is indistinguishable from a broken detector**; liveness is asserted and the sentinel covers it. (g) Log scoping is by **timestamp**, never byte offset — the logger archives on startup. All four value detectors mutation-tested: each mutation makes the same captured evidence stop satisfying it. ⚠ **WHAT THIS DOES NOT COVER:** **1 of the 14** delivery sites was exercised live (the inject bootstrap, chosen because it needs no game, no pipe and no CE). The other 13 share `ClipboardDelivery.TryAsync` and `FailureText`, but their FALSE branch lands on **three different UI surfaces** — 7 on `ErrorMessage` via `SetError`, 6 on `StatusText`, 1 on `CoordStatus` — so this run proves the mechanism, not all 14 renderings. See `[CLIPELLIPSIS-2026-09-09]` in todo.md for what the toolbar rendering costs. |
| **SW3** | ✅ **CLOSED 2026-09-09 `[SW3-UNTICK-2026-09-09]`** — CE Lua untick + Invoke's `onUnreadable` (`b344d63d`, `a55ef2b5`). | **BOTH failure modes produced, in real Cheat Engine, with the artefact under test being `InvokeScriptGenerator`'s own output** — pushed straight into CE's table by the UI's `INV` button via the AOBMaker plugin, never hand-written. `a55ef2b5` split two failures the script used to conflate, and the point of the row is that it can tell them apart:
&nbsp;&nbsp;**(1) mailbox UNREADABLE** — DumperTest killed while CE kept the process open, then the record ticked: *"[Invoke] the contract symbol resolved to 7FFD39896F90 but that memory could not be READ. Is Cheat Engine still attached to the game?…"*
&nbsp;&nbsp;**(2) mailbox genuinely BUSY, DLL ALIVE** — `tools/verify/sw3_mailbox_busy.py`: every thread suspended so the DLL's own poller cannot clear it, `cmd` written to `0x7FFFFFFF` with `WriteProcessMemory`, then the record ticked: *"the DLL mailbox is busy — another CE script or a previous invoke is still mid-command; try again in a moment — instance lookup not attempted"*.
**The record UNTICKED on both**, which is the deferred-untick half. Two manufactured conditions, two different correct messages, and the busy one even says what it did NOT do ("instance lookup not attempted") — that is "the script must not be guessing" measured rather than asserted. ⛔ **THE SUSPEND IS NOT INCIDENTAL.** The first attempt just wrote `cmd` through `write_mem` and read back **0**: the DLL's poller rejects an unknown command id and CLEARS it within the second. Spin-writing cannot work either — the Lua loop is `while _idleCmd ~= 0`, so it exits on the first zero it samples. Suspending stops the poller while leaving memory readable, which is all CE needs; and the write then cannot go through `write_mem` at all, because that command is served by the pipe thread INSIDE the suspended target. ⛔ **AND TWO OF THE THREE "SITES" ARE DOCUMENTATION — verified by hand.** Both `scripts/ue5_invoke_helper.lua:48` and `scripts/ue5_freeze_helper.lua:174` sit inside `--[[ … ]]` block comments. ⚠ **NOT a gate defect** — `check_ce_untick_placement.py:105` says "Block comments INCLUDED" and its header gives the reason: these copy-me samples themselves taught the wrong pattern for five weeks after the C# generators were fixed. The consequence for this row is only that those two cannot be exercised by running anything. The executable sites are the five C# generators plus `scripts/UE5CEDumper.CT:884`. |
&nbsp;&nbsp;*"[Invoke] the contract symbol resolved to 7FFD39896F90 but that memory could not be READ. Is Cheat Engine still attached to the game? If the process is running, re-open it in CE, then untick and re-tick this table."*
— a DEAD/unreadable process, **not** a busy mailbox, which is the distinction `a55ef2b5` exists to make — and **the record unticked itself**, which is the deferred-untick half. Neither had ever been observed. ⛔ **AND TWO OF THE THREE "SITES" ARE DOCUMENTATION — verified by hand.** Both `scripts/ue5_invoke_helper.lua:48` and `scripts/ue5_freeze_helper.lua:174` sit inside `--[[ ... ]]` block comments (invoke_helper opens on line 1 and closes on line 67). ⚠ **This is NOT a gate defect** — `check_ce_untick_placement.py:105` says "Block comments INCLUDED" and its header gives the reason: these copy-me samples themselves taught the wrong pattern for five weeks after the C# generators were fixed. The consequence for THIS row is narrower: those two cannot be exercised by running anything, because there is nothing to run. The executable untick sites are the five C# generators plus `scripts/UE5CEDumper.CT:884`. **WHAT IS STILL OPEN:** the DLL-side half as the row words it — *"the mailbox `status` field must say which failure it was"*. That arm needs the DLL ALIVE and the mailbox returning an error, which is a different manufacture from this one (here the DLL is gone, so there is no status to read — and the script correctly said so rather than guessing, which is the half that was in doubt). |
| **SW4** | ✅ **CLOSED 2026-09-09 `[SW4-CEPAD-2026-09-09]`** — the CE-side delegate pad (`[D4B-TAIL]`, `[STALE-NONE]`). | **PASS, on a CHECKED build**, `tools/verify/sw4_ce_delegate_pad.py` + `tools/verify/ce/ue5-sw4-record-dump.lua`, DumperTest 5.4 Development with the real Cheat Engine attached. The DLL half was fixed and measured earlier the same day (`[SW7]` found `delegate_pad` had never reached the wire at all); this is CE resolving one of those records and someone reading the answer. ⭐ **The numbers, all four of them.** Field `Multicast_Inline` at offset `0x980` on an actor at `0x1ED06BBCAE0`, `delegate_pad=8`:
&nbsp;&nbsp;• the exported CE XML reads `<Description>"Multicast_Inline (980)"` with `<Address>+988</Address>` — the description carries the TRUE field offset while the address carries `offset + pad`, which is `CeOffset()` doing its job;
&nbsp;&nbsp;• `[0x1ED06BBD460]` (unpadded, the access detector) reads **0x0**;
&nbsp;&nbsp;• `[0x1ED06BBD468]` (padded, `InvocationList::Data`) reads **0x1ED33872F80**;
&nbsp;&nbsp;• Cheat Engine resolved the pasted group to **0x1ED33872F80** — 2 records on the padded side, **0 on the unpadded side**.
So the group resolves to the InvocationList, and an unpadded record would have resolved to **address 0** — which is this row's acceptance verbatim. The scalar `Del_Unicast` (a flat `8 Bytes` leaf, no `<Offsets>`) resolved to `0x1ED06BBD480`, the padded address, not the detector at `0x1ED06BBD478`. ⚠ **The two shape corrections were right and are now measured, not assumed:** a scalar `DelegateProperty` really does emit a flat leaf rather than a group, and the multicast row really is a GROUP HEADER with `<Offset>0</Offset>` — a POINTER record, so what CE reports is the DEREFERENCE of the padded address. The rig compares both candidates and their derefs for that reason. ⛔ **TWO SIDE-FINDINGS, filed not fixed** — the OTHER two CE-facing paths in the Live Walker do not carry the pad: the `HEX` button logged `AOBMaker: navigated hex view to 1ED06BBD460`, the **unpadded** address (it uses `FieldAddress`, which the grid's Address column also shows unpadded); and the per-row `+CE` button does **nothing at all** on a delegate row — no record, no log line, no error — while working normally on a scalar field one row away (`WideGuard` → `1ED06BBD3E4`). See `[CEPATHS-UNPADDED-2026-09-09]` in todo.md. |
| **SW5** | ✅ **CLOSED 2026-09-09 `[SW5-PUSHBRANCHES-2026-09-09]`** — `PropertyXrefDialog`'s push-result branches (`c55f2237`). | **BOTH previously-unreachable branches produced, both sides**, `tools/verify/sw5_push_branches.py` + `tools/verify/ce/ue5-sw5-synchronize-split.lua`, on **EVERSPACE 2** (812,548 objects) with the REAL AOBMaker CE plugin. Before `c55f2237` all three outcomes printed the same green label; only GREEN had ever been seen since. ⭐ **The mechanism, and why it is exact rather than a stand-in.** Both plugin handlers run their work through CE's plain Lua global `synchronize` — the record via `LuaDoBuffer` with chunkname `=AOBMakerCEBridge` (`pipe_server.cpp:1063-1078`), the navigation via `luaL_dostring` whose chunkname IS the script text and contains `DisassemblerView` (`:912-918`). `synchronize` is `lua_register`ed (`LuaHandler.pas:16290`) so it is overwritable, and the plugin's state is a `lua_newthread` of the main VM (`LuaHandler.pas:188`) sharing `_G` — so an autorun wrapper is seen by the pipe worker, and `debug.getinfo(2,"S").source` splits the two callers. **The real plugin does the real work, minus the one call we refuse.** **AMBER** (`refuse-navigate`): CE side `caller=record action=passed` / `caller=navigate action=refused`; UI side *"Added SetEyeTrackedPlayer to the CE table @ 0x7FF65858A11C, but could not open the disassembler there — find the record in Cheat Engine and browse to it manually."* **RED** (`refuse-record`): CE side `caller=record action=refused` / `caller=navigate action=passed`; UI side *"CE refused the push for SetEyeTrackedPlayer — nothing was added to the table. Is Cheat Engine still open with the AOBMaker plugin loaded?"* ⭐⭐ **AND CE ITSELF CORROBORATES EACH MESSAGE'S CLAIM**, because the harness leaves CE ALIVE and samples `getAddressList().Count` on every call: the AMBER run went **0 → 1** (a record really was added, as its text says) and the RED run stayed **0 → 0** ("nothing was added to the table", as its text says). That contrast is the row's two-sidedness in one number. ⛔ **"JUST CLOSE CHEAT ENGINE" DOES NOT PRODUCE RED**, which is why this needed a harness rather than a `taskkill`: `_btnDisasm` is enabled only while `SharedAobMaker?.IsAvailable == true` (`:387`) and the handler re-read the same cached flag (`:471`), so with CE gone the click was a no-op. ⚠ That was a defect of its own — `IsAvailable` is refreshed only by user-triggered probes and this dialog is MODAL, so killing CE with it open left a stale `true` and the click vanished **silently**. Fixed the same day: it now reports in red, with wording distinct from this branch ("Cheat Engine is not reachable — nothing was pushed"). ⛔ The harness is **INERT without its job file** and `uninstall` removes both files from the user's real CE install — always run it. |
| **SW6** | ✅ **CLOSED 2026-09-09 `[SW6-STRIDEREFUSAL-2026-09-09]`** on the two ARRAY arms; the two SCALAR arms are **measured unreachable** and the row is amended. | The register said these were reachable "only on an engine whose delegate ElementSize is neither *base* nor *base+8*", i.e. never — 5.4/5.7/5.8 and UE 4.23/4.27 all report one of the two across 600 classes each. ⭐ **The house technique reaches them anyway.** The size the ARRAY readers validate is the ArrayProperty **Inner**'s `FProperty::ElementSize`, read LIVE on every walk (`Ubel.cpp:4370-4372`), and the wire already hands over that Inner's address as `array_inner_addr`. **PASS**, `tools/verify/sw6_stride_refusal.py`, DumperTest 5.4 Development: one 4-byte write of **32** (neither 16 nor 24) to `Inner + fproperty_elemsize`, and both readers refused for the first time in this repo's history. **DLL/log side:** `[WARN] [WALK] ReadMulticastDelegateArrayElements: ElementSize=32 is neither 16 nor 24 (+detector) -- refusing to guess a stride`, and the same from `ReadDelegateArrayElements`, both timestamp-scoped INSIDE the poke window. **Wire/UI side:** `array_elem_size` reads back 32 and the `elements` key **disappears** where two elements stood. ⭐ That is a STRUCTURAL observable, not a render echo of a string the DLL built — which is what makes the array arms the better evidence here. Restore byte-exact on both. `DelegatePadFromElementSize(32, 16) < 0` was already unit-pinned (`dll_helpers_test.cpp:6590`); what had never happened is a CALL SITE reaching it. ⛔⛔ **THE ROW'S ACCEPTANCE TEXT WAS UNSATISFIABLE, AND THE CODE — NOT THE TEXT — WAS FIXED.** It asked for "the `WALK` warning naming the size" on the DLL side AND the rendered refusal string on the UI side, and **no single arm could produce both**: the two ARRAY sites warned but were silent on the wire's value side (`Ubel.cpp:4515` DISCARDED the reader's `error`, and an ArrayProperty never sets `typedValue`, so a refused array rendered as a bare header — indistinguishable from an empty one), while the two SCALAR sites rendered their string but emitted **no log line at all**. ⭐ Both gaps closed 2026-09-09 after this rig measured them: the array refusal now lands in `typedValue`, and all four sites warn. **Re-verified live** — the wire now carries `(multicast array — unexpected multicast element size 32, not read)` and `(delegate array — unexpected FScriptDelegate element size 32, not read)` alongside the WALK warnings, so both sides of the charter come from the SAME arm. ⚠ **WHY THE SCALAR ARMS ARE UNREACHABLE — measured, not assumed.** They validate `fi.Size`, read once by `EnumerateFields` and memoised by `WalkClass` into `s_walkClassCache` (2048-entry LRU, **no invalidation command anywhere in the DLL**). Evicting it is the obvious way in, and it does not work: on a COLD process, `walk_class_batch` over 600 classes fills the LRU to `entries=2048 fields=29251`, and over ALL 3,946 leaves it **byte-identical**. `walk_class_batch` goes through `WalkClassEx`, which consults the separate **UNBOUNDED** `s_walkClassExCache` (`Ubel.cpp:1200`) and calls `WalkClass` only on a MISS — so once that front cache is warm the LRU stops taking inserts and therefore stops evicting, and our class never reaches the back. `--scalar` runs the probe and FAILS with that finding rather than assuming it; see `[CLASSCACHE-FRONTED-2026-09-09]` in todo.md. |
| **SW7** | ✅ **CLOSED 2026-09-09 `[SW7-STALEARM-2026-09-09]`** — the `(stale)` narrowing's stale ARM (`0b8f89e5`). | **PASS on both configurations**, `tools/verify/sw7_stale_arm.py`, DumperTest 5.4. Manufacture-and-restore: `ResolveWeakObjectPtr` returns 0 on exactly two conditions, and one of them (`GetSerialNumber(idx) != serialNumber`) carries the comment `// stale reference` — so writing `serial + 1` into the stored `FWeakObjectPtr` manufactures the engine's OWN staleness, leaving `objectIndex > 0` and the FName untouched. ⛔ `serial = 0` would have been a silent no-op; `+1` is unconditional. **DLL side:** the element's resolved `pa`/`pn` keys disappear from the payload — the DLL saying the target is gone, not the renderer choosing a string. **UI side:** `Del_Unicast` and `Arr_Delegates[1]` both go `DumperTestActor_0::D4b_OnPingProbe` → `(stale)::D4b_OnPingProbe`, while the never-poked `Arr_Delegates[0]` stays `(unbound)`; restore returns the baseline byte-for-byte. Both render sites were exercised — the scalar handler and `ReadDelegateArrayElements` — on Development (pad 8) and Shipping (pad 0). ⭐ **This rig found a SHIPPED bug on its first run** — see SW4. ⛔ The standing warning still holds for any future rig: **do not locate an ARRAY element via `delegate_pad`**; it is absent there, and deriving the address from it lands in the access detector and passes while measuring nothing. Derive from `array_elem_size`. |
| **SW8** | ✅ **CLOSED 2026-09-09 `[SW8-NOSPURIOUS-2026-09-09]`** — `Ubel::GetMapPairLayout`'s refusal (`[TMAPGEOM-2026-09-09]`). | The refusal ITSELF was already proven, but **OFFLINE by construction** — `dll_core_test` manufactures the partial read with a page edge (`VirtualAlloc`, one page committed of two), because a wholly-unreadable property short-circuits earlier and no live game can be made to unmap one page of a property. What was owed is the OTHER direction, which every new refusal owes: **that it does not fire when nothing is wrong** — a refusal that blanks working data is a regression even with a green unit test. **PASS**, `tools/verify/sw8_map_geometry.py`, on a real shipped title: **Titan Quest II (UE 5.7, Shipping, save loaded)** — 586 TMap fields over 353 classes, **1,685 distinct map-derived schema leaves, 815 of them both-struct**, and **0** refusals across 8 walk logs carrying 8 `[WARN] [WALK` lines. Also PASS on DumperTest 5.4 (350 leaves / 101 both-struct / 0 refusals / 225 logs / 37 WALK warnings). **DLL side:** the leaf count — `CollectSchemaLeaves` recurses into a map only inside `if (GetMapPairLayout(...))` and only into `layout.valueStructAddr`/`keyStructAddr`, the very pointers the guarded block reads, so one such leaf proves the guarded `ReadSafe` ran, succeeded, and published a layout; a `.Key`/`.Value` tag proves BOTH came back non-zero. **Log side:** zero refusal lines, on a channel shown to carry WALK warnings. Both failure arms mutation-tested red. ⛔ **`walk_instance` IS NOT THIS FUNCTION** — the instance walk carries its own inlined copy of the map geometry (`Ubel.cpp:~4730`) and never calls `GetMapPairLayout`, so `map_stride` on the wire is not evidence about it; the four real callers are `CollectContainersRecursive` / `CollectRefMetaRecursive` / `CollectSchemaLeaves` / `ScanForValue`. ⛔ **A UE < 4.25 title makes this vacuous** — `GetMapPairLayout` returns false at its first line when `bUseFProperty` is false, so every map is refused before the branch under test and the run shows a clean zero while measuring nothing; the rig asserts `use_fproperty` and says why. ⭐⛔ **AND THE FIRST ATTEMPT WOULD HAVE BEEN A FALSE PASS**: TQ2 runs our code through a **deployed proxy**, and the deployed `dxgi.dll` predated `[TMAPGEOM]` — `"TMap geometry"` was **not a string in that binary at all**, so the refusal count was guaranteed 0 for a reason having nothing to do with correctness. Caught by grepping the deployed DLL for the refusal text before trusting the run; fixed with `proxy_refresh.py`. **Grep the artefact under test for the string you are counting the absence of.** |
| **SW9** | ✅ **CLOSED 2026-09-09 `[SW9-UE4READ-2026-09-09]`** — the UE4 fixture had no READ test (`[D4B-UE4]`). | **PASS on BOTH UE4 arms**, `tools/verify/sw9_ue4_delegate_read.py`. The pad survey walks CLASS field tables, so it needed no instance — `walk_instance`, `find_instances` and `invoke_function` appeared in **ZERO** UE4 log, so no UE4 run had ever read a delegate's BINDINGS. Now they have, off an actor the **engine** spawned and bound: `UCheatManager::Summon("DelegatePadFixture")` over `invoke_function` with `str_params` (a real by-value FString in the game heap, `direct_call` FALSE so the spawn runs on the game thread). ⭐ Deliberately NOT a manufactured binding — the READ is what is under test, so the FNames must be the engine's, made by `BeginPlay`'s `AddDynamic` / `BindDynamic`. **UE 4.27** (`use_fproperty=True`) and **UE 4.23** (`use_fproperty=False`, the UProperty path — the row's headline concern) both give `Multicast_Inline` and `Del_Unicast` = `DelegatePadFixture_2147482531::DPad_OnPingProbe` and `Arr_MulticastDelegates` = `[(0 bindings), (1 binding) [..::DPad_OnPingProbe]]` at `array_elem_size` 16. **DLL side:** the spawn is a **difference, not an assumption** — BEFORE shows only `Default__DelegatePadFixture`, AFTER shows a new non-CDO instance. **UI side:** Live Walker on that same address (`0x18B90CCCB80`), `Del_Unicast` tooltip `DelegatePadFixture_2147482531::DPad_OnPingProbe`; the array drills in to `[0] (0 bindings) @0x18C2FC2DB00` and `[1] (1 binding) [..::DPad_OnPingProbe] @0x18C2FC2DB10` — the 0x10 stride visible on screen. ⛔ **Read the TOOLTIP, not the cell** (`[V8PREVIEWCLIP]`): the Value cell is a fixed 200 px TextBlock with no ellipsis, and it clipped to `DelegatePadFixture_2147…` — a screenshot of the grid alone would NOT show the probe name. ⛔ **TRAPS THE RIG ASSERTS.** (a) `invoke_function`'s `class_name` shortcut falls back to the **CDO** with only a LOG_WARN (`Frieren.cpp:1038-1044`); on the CDO `GetOuterAPlayerController()` is null so Summon does nothing while still answering `result=0` / "ProcessEvent OK" — a guaranteed false pass. The rig resolves a non-`Default__` CheatManager itself and passes `instance_addr`. (b) Pre-fixture UE4 builds share the exe stem **and the log folder**, and are within 5,632 bytes of the fixture builds; the exe is BYTE-SCANNED for `DelegatePadFixture` before launch, and injection is BY PID. Both guards mutation-tested against the real binaries they exist to reject. (c) `launch_dumpertest.py`'s one-game-at-a-time guard is **blind to UE4 images** — the rig carries its own. ⚠ **WHAT THIS DOES NOT COVER, stated because coverage is the point:** the stride arm CANNOT fail on UE4 (no access detector, so ElementSize == base and the pad is 0 in every UE4 configuration) — it is asserted as RECOGNITION + INDEXING only, and `delegate_pad`'s absence is **vacuous** on UE4 (Fern emits it only when non-zero), so it is printed and never asserted. And the fixture declares no `TArray<FScriptDelegate>`, so `ReadDelegateArrayElements` — Phase J, "the sixth D4b site" — and the sparse walker are **never executed** by either arm. The rig prints the readers it actually touched. ⚠ **"Nothing spawns it" understated what is available, corrected 2026-09-09**: `UCheatManager::Summon` is a reflected `UFUNCTION(exec)` and the CheatManager is live in a standalone **Development** package, `invoke_function` already carries FString params, and `DelegatePadFixture` / `DPad_OnPingProbe` are byte-verified present in both packaged UE4 Development exes — so a genuinely BeginPlay-bound instance is one pipe call away with no rebuild. ⛔ Do NOT substitute a manufactured binding: that renders SW7's arm wearing SW9's string, and reads an FName we wrote rather than one the engine made. `install.py --spawn-from` still prints the two-line patch for the repackage route. |
...
### ℹ️ DEFERRED, NOT A VERIFICATION ITEM — AB23: intern `GroupSlotMatch::ownerClass`

> ⛔ **MARKER CORRECTED 2026-09-06.** This heading carried a ⬜ and was therefore counted as an open **live-game** batch, which it never was — the row's own second sentence says *"this is unshipped work, not something awaiting a live check"*. There is no observable to verify because **the code was never written**. Two of its own justifications are also now stale:
> * *"it is counted by `check_audit_register.py`"* — that gate was **RETIRED 2026-09-03** (`tools/check_all.py:47`). Nothing counts it now.
> * *"no test target compiles `Aura.cpp` or `Fern.cpp`"* — **`Aura.cpp` has been compiled by `dll_core_test` since 2026-08-25** (`dll/tests/dll_core_test.cpp:71`). Four of the six sites are the `Aura.cpp` writes, so the refactor is now mostly verifiable **offline**; only the single `Fern.cpp` read is not. The deferral's stated reason no longer holds as written — re-decide it on that basis rather than inheriting it.
...
> | G7 | ⛔⛔ **UNREACHABLE BY CONSTRUCTION — CLOSED AS UNFALSIFIABLE 2026-09-06 `[G7-UNREACHABLE-2026-09-06]`. Do NOT spend a game boot on this; no title can produce the line.** The `validated=NO -> YES (re-run)` message (`Genau.cpp:4208`) is guarded by `s_lastPublishedValidated != -1` (`:4207`), so it needs `ValidateAndFixOffsets` to reach a publish **twice in one process**. There are exactly two call sites and they are **mutually exclusive**: `Frieren.cpp:355` (init) sits inside `if (ptrs.GObjects && ptrs.GNames)` (`:223`), and `Fern.cpp:5161` (apply_rescan) is gated on that *same* predicate **plus** `!DynOff::bOffsetsProbeRan` (`:5160`) — while **every** exit of `ValidateAndFixOffsets` stores that flag true (`Genau.cpp:3660`/`:3781`/`:4195`, covering the returns at `:3661`/`:3782`/`:4257`; the other six `return`s in the span are nested lambdas). So: init ran ⇒ apply_rescan is gated off ⇒ never a second publish. Init skipped ⇒ apply_rescan's call is the **first** publish ⇒ `s_lastPublishedValidated == -1` ⇒ no line. ⭐ **`!bOffsetsProbeRan` is audit #5 G3, shipped in THIS SAME L1 batch — G3 removed the only path G7's observable had.** `Fern.cpp:5155-5158` states the invariant out loud without drawing that conclusion. The hunt below was therefore chasing a case that could not have produced the line even if a title had been found; it is kept because the nine-title sweep is real evidence about something else (every title validates on the first pass). The correct output here is a code change (a staged one-shot hook, the technique `tools/verify/g3_rescan_apply.py` already uses) or deleting the observable — not a fixture hunt. Verified line-by-line from the primary sources 2026-09-06. *(original:)* **NOT REACHABLE HERE, measured 2026-08-19 `[G7-NOSAMPLE-2026-08-19]`.** The step needs a title whose offsets validate **only after a re-scan**, so that the `validated=NO -> YES (re-run)` transition exists to observe. **All NINE titles swept tonight reported `probe_ran=true, validated=true` on the FIRST pass** — Lushfoil, Manor Lords, Solarpunk, EVERSPACE 2, Geri, Avowed, DQ7R, Elliot, OCTOPATH. ⚠ That includes **Solarpunk, which this row names as the example**; it validates immediately today, so the row's own suggested host no longer produces the case. Until a title that fails first-pass validation turns up, there is nothing to transition *from*. (Original step kept below.)<br><br>~~on a game that offsets-validates only after a re-scan (e.g. **Solarpunk**), connect, then trigger **apply_rescan** (the pipe/UI re-scan path)~~ | the DYNO/offsets log gains a `validation state CHANGED validated=NO -> validated=YES (re-run)` line and the summary header reads `=== Dynamic Offset Summary (validated=YES) ===`; `get_offsets` and the log now agree | before, the one-time UE5_Init scan-log summary said validated=NO forever while live state was true |
...
> | A7 | ✅ **CLOSED 2026-09-06 `[A7-FINDBYADDR-2026-09-06]` — OFFLINE, and the previous closure was against the WRONG LOOP.** `[A7-CORETEST-2026-08-25]` added two `dll_core_test` blocks labelled A7 that drive `Aura::ForEach` — but ForEach **already had** its poll; it is one of the *siblings* A7 was written to match. A7 fixed **`FindByAddress`** (`Aura.cpp:1867`), which hand-rolls its own loop and never calls ForEach, exactly as the audit row says (`audit-2026-08-13-early-code-findings.md:278`). Measured 2026-09-06: `grep FindByAddress dll/tests/ tools/verify/` = **zero hits** — the shipped fix had no coverage at all while two documents called it verified. Now four checks drive `FindByAddress` against the in-process fake pool, and they are **proven non-vacuous by deleting the poll**: without it the block fails with `...reports no index rather than a stale one   got: 8000` — found *while cancelled*. No game needed; the live step below is superseded. *(original step:)* on a huge game, start a **find-object-by-address** (get_ce_pointer_info / find_by_address triggers `FindByAddress`) and **disconnect the client mid-scan** | shutdown/next command is prompt — no multi-second hang while the full GObjects walk finishes; the lookup returns "not found" | the loop now polls `Tot::Requested()` every 0x1000 objects like its siblings; only observable under a real disconnect on a large pool |
...
> | G6 | ✅ **CLOSED 2026-09-06 `[G6-TRISTATE-2026-09-06]` — OFFLINE, no MindsEye needed.** `Macht::ReadSafe` reads **this** process, so a `std::vector` shaped like the fork's hash table is indistinguishable from the fork's own — the same trick the fake `FUObjectArray` already uses. 9 checks in `dll_core_test`, staging the ctx (`+0x10` entries, `+0x18` count, `+0x44` sentinel, `+0x50` buckets, `+0x58` capacity; 24-byte entries) and driving the real `Serie::InitObfuscated` entry point. **Every check is a PAIR — the same tag before and after the table settles — because the finding is that the two kinds of miss must get OPPOSITE treatment.** ⭐ **Red-tested twice, and the two arms discriminate independently:** making a transient miss cache (the old `TAGKEY_MISS` bug) fails **5** checks, all in the transient arm; making a genuinely-absent tag fail instead of resolving to plaintext fails **3**, all in the absent arm, while the transient arm stays green. `Serie.cpp` restored byte-identical after each. ⚠ **SCOPE:** this pins the **tri-state logic**, not that the ctx offsets match MindsEye's real table — that half came from RE and can only be confirmed on the fork. G6 was filed as a *logic* defect, so the logic is the finding. ⚠ Also measured: `grep -rn 'TagKey|Obfuscated' dll/tests/ tools/verify/` returned **zero** hits before this — the fix had shipped 2026-08-19 with no coverage of any kind, the same shape as A7. *(original:)* (obfuscated fork only — **MindsEye**, no sample here) let name resolution race the fork's live key-table growth; also view a block whose tag is genuinely **absent** from the table | a transiently-unresolvable tag recovers on a later name (no permanent blanking of every FName with that tag); an absent-tag block renders as plaintext | the tri-state `LookupTagKey` no longer caches a transient miss, and a clean-absent resolves to key 0 (plaintext) per Genau's rule |
