...
* **A local LLM, opted in per machine.** `py tools/llm/ollama_local.py status` says whether THIS PC
  has one (`ready`) or not (`disabled`: nothing to do). The model tag lives in the gitignored
  `.claude/local-llm.json` and the game-VRAM hook in the USER-level `~/.claude/settings.json`
  (every repo's sessions), never in the repo; to opt a machine in, see
