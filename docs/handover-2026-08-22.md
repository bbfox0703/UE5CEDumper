...
| branch | `dev`, **ahead of `origin/dev` and deliberately NOT pushed** — derive with `git status -sb` |
| CI gates | **12 / 12 green** (`py tools/check_all.py`, 6 s) |
| C# tests | **4,693 / 4,693** |
...
⚠ **Why `dev` is unpushed**: a PR runs CI, and CI runs a full clean Native-AOT publish plus the whole
suite over work the maintainer has not reviewed. **Do not push or open a PR unless asked.** The
convention is commit to `dev` → `gh pr create --base main --head dev` → `gh pr merge N --merge`,
never `--admin`.
...
### ⭐ First: they are already held. Check before you request.

**The plan doc's claim that "grants do not survive a session" is FALSE on this machine.** Measured
2026-08-22: `list_granted_applications` returns **20 grants made on 2026-08-19 19:31–19:36**, still
valid three days and many sessions later.
...
nothing. ⚠ Grants **are** lost by a **reboot** — that is the real invalidation event, not a new
session.

### The 20 that are currently granted
...
* ⚠⚠ **The eight Steam-game grants are keyed to `steam://rungameid/<appid>`, NOT to the shipping
  exe.** Nothing currently grants e.g. `Elliot-Win64-Shipping.exe`. So a row that needs to *click
  inside the game* implies a **mid-run `request_access` for the running exe** — the one thing the
  preamble above says stalls an unattended batch. Plan it: launch the title, request the exe while
  it is up, then start the row. Rows that only inject and read the pipe need no such grant.
...
* **`Titan Quest II` appears in the plan doc's Batch 1 but is NOT granted**, and its
  `appmanifest_1154030.acf` has `StateFlags 6` (not the `4` that means fully installed).
...
```bash
py tools/check_audit_register.py --list          # audit #5 register
grep -c '^> | `\[' docs/todo.md                  # OPEN FIXES INDEX rows  -> 4
grep -c '^### ⬜ NEW DEFECT' docs/todo.md         # defect WRITE-UPS       -> 1 (not the queue!)
awk '/^## Pending live-game verification/,0' docs/todo.md \
