...
| 6 | **`steamwebhelper.exe`** | ⭐ owns the Steam **library window**; without it the whole Steam UI is a masked black rectangle. **Pass the `.exe` suffix** — a bare `steamwebhelper` resolves back to `Steam` | full |
...
⛔ **A folder under `steamapps/common` is not an installed game — 11 of 74 here hold no exe at all**,
including **FINAL FANTASY VII REBIRTH** and **Tower of Mask**, both cited in `docs/` as fixtures for
open rows. Run this before planning a session around a title.
...
grep -c '^### ⬜ NEW DEFECT' docs/todo.md                                  # defect write-ups
```
...
⭐ **The cheapest real work is the 繁中 checklist**, and `第 1 步` / `第 2 步` hold rows that need far
less than their bucket implies:

* **`AC15` step 1 needs no game at all** — Proxy Deploy: Steam-library scan vs drive scan must find
  identical games. It is a UI-only row sitting in the inject-a-game bucket.
* **`AF16–AF23` step 2** needs a game **with Blueprint bytecode** (DumperTest's `Funcs` column is
  empty). ⚠ Must be run against a `-Mode Publish` **AOT-trimmed** binary — a dev build gives a false
  PASS, because the reflection-based sort survives JIT and is trimmed only in what ships.
* **`W1`/`W7`** (a real parser must read our `.usmap`) is unblocked — the maintainer approved
  CUE4Parse as a NuGet package **in a throwaway project only**; it must not enter `UE5DumpUI`'s deps.
* **`M1–M5` step 1 arms (a) and (b)** need a human moving / stalling the game; arms (c) and (d) are
  done. The rig is written: `py tools/verify/seethrough_arms.py run`.
* **`A6` step 5's spawn half** needs a game where a level reload or an enemy respawn can be
  triggered — DumperTest has no repeatable in-process object-recreation lever (measured three ways).
...
* ⚠ **ProcessEvent vtable slots are per-BUILD**: `0x268` DumperTest 5.4 · `0x260` Lushfoil 5.6 ·
...
* ⚠ **`Serie::GetString` drops the FName `Number`** — 40 of 42 objects are named differently by two
  pipe commands and 6 of 6 are unfindable by the name the DLL itself reports. Blast radius measured;
  71 call sites, only 4 pass a Number. ⛔ **Do not sed it** — display vs identity are different jobs.
...
3. **Tick every individual row** in the audit doc, not just the grouped one, and run
   `py tools/check_audit_register.py`.
4. **New method lessons go in [`working-lessons.md`](working-lessons.md)** — single copy, travels with
   git. Not into memory files, which do not.
5. **`dev-log.md` is append-only, newest first** — never edit a past entry, even to fix a stale
   number.
6. `py tools/check_all.py`, then commit.
