...
* ⚠ **ProcessEvent vtable slots: the PATTERN SCAN is primary and is what has to work. There is now
  ...
  ⚠ The ES2 figure is recorded **only here** — it has no other home in `docs/`, so do not delete
  this line looking for a better source. It predates 2026-08-22 and therefore ran against the
  **5.5.4** build (see the ES2 sub-bullet below for how that was established).
