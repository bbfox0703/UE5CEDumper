...
* **A local LLM, opted in per machine.** `py tools/llm/ollama_local.py status` says whether THIS PC
  has one (`ready`) or not (`disabled`: nothing to do). The model tag and the game-VRAM hook live in
  gitignored `.claude/` files, never in the repo; to opt a machine in, see
  [the skill](../.claude/skills/local-llm/SKILL.md) §6.
