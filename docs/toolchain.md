...
| **patternsleuth** | already on disk: `vendor/RE-UE4SS/deps/first/patternsleuth` (RE-UE4SS's submodule, synced by `sync_tools.ps1`; `1d90b02` at vendor audit #7) | Not on crates.io and has no releases. Build the CLI from that directory so its `rust-toolchain.toml` pin applies. Do not `cargo install --git` — rustup reads the pin from the cwd, so it would be ignored. A separate clone is no longer needed `[VND583-DOC PS-11]`. ⚠ Its AOBs are not re-scanned into Himmel unless upstream adds new ones (maintainer, 2026-09-24). |
