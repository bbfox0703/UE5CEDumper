...
| [wiki-zh-TW-style.md](wiki-zh-TW-style.md) | 🇹🇼 **Before writing or re-translating a zh-TW Wiki page** — the rules (English first, translate from the source, UI names stay English) and the glossary derived from the maintainer's own hand edits. |
| [wiki-ja-JP-style.md](wiki-ja-JP-style.md) | 🇯🇵 **Before writing or re-translating a ja-JP Wiki page** — JTF + Microsoft base, the half-width-space decision, long-vowel rule, glossary (row/column is the OPPOSITE of zh-TW), and the textlint check. The maintainer cannot read Japanese: keep the reviewer + textlint steps. |
...
| [review7-live-plan.md](review7-live-plan.md) | **Before running any live check of a Review 7 row** (`todo.md` `[REVIEW7-2026-09-24]`) — the 26 host sessions in order, the pre-fix binaries each red arm needs, and each row's status; the per-row recipes are in `review7-live-recipes.json`. ⛔ Update its status in the same commit as the row's record in `todo.md`. |
| [fixpass-low-live-plan.md](fixpass-low-live-plan.md) | **Before running any LOW row of the fix pass's live-check backlog** — which rows share a fixture launch, in what order, each row's status, and a per-row recipe. ⛔ Update its status in the same commit as the row's record in `todo.md`. |
| [path-shape-live-plan.md](path-shape-live-plan.md) | **Before running any live check of `[PATH-SHAPE-2026-09-25]` or `[PROXY-DEPLOY-UX-2026-09-25]`** — the five sessions in order, what each proves, each row's status; path shapes come from `tools/verify/path_shape_folders.py`. ⛔ Update its status in the same commit as the row's record in `todo.md`. |
...
| [audit-2026-09-24-vendor-ue583.md](audit-2026-09-24-vendor-ue583.md) | **Vendor audit #7** — UE 5.8.3 changed nothing for us (measured on the repackaged DumperTest58 too); the value is the older defects that reading UEPseudo and the RE-UE4SS per-game configs for the first time exposed — FunctionFlags keyed on version is wrong on FF7R and DQ XI S. minhook: HOLD. ⚠ Six patternsleuth rows are UNVERIFIED. |
...
| [../tools/llm/README.md](../tools/llm/README.md) | **The local-LLM helper, for any repo on the machine.** Installed once per machine; another repo joins or leaves with one command and receives only a skill file that carries no machine fact. Read it before adopting the helper elsewhere or changing how it installs. |
...
| [comment-integrity-eval.md](comment-integrity-eval.md) | **Code comments vs the code — EVALUATED 2026-09-26; items 1 + 2 (gates, cleanup) DONE the same day, §5.** Measured: 69% of in-repo `File:line` refs have drifted, about 26% of substantive comment blocks are stale (DLL 30-33%, UI 13%), the cause is additive drift in comments that enumerate. Read before a comment cleanup or a new comment gate; the rigs are `tools/verify/comment_audit.py` / `comment_sample.py`. |
