...
- 🟡 **`B29 step 3`** — **fixture BUILT 2026-08-24** (`tools/verify/b29_nonascii_fixture.py`, `[B29-NONASCII-FIXTURE-2026-08-24]`): a real UE game at `D:\測試\DumperTest` with a third-party ReShade `dxgi.dll` loaded from it. The run itself needs our DLL **registered as a CE plugin** — a persistent CE config change that re-creates the `[STALEDLL]`(a) state, so it is a maintainer decision. ⚠ A `mklink /J` junction does **not** work here even though it closed `AC17`/`VOLUMEROOT` — Windows resolves the reparse point and the process reports the real ASCII path. ⚠⚠ Step 3 is **predicted to FAIL** from source, `[NONASCIILS-2026-08-24]`: the message uses `%ls` through a narrow `vsnprintf` with no `setlocale` anywhere in `dll/src`, while `Heiter.cpp` was explicitly fixed for this by converting with `Utf8Helpers::EncodeUtf16` first. Six `%ls` path sites share the shape.
