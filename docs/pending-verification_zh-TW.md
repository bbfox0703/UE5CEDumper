...
| # | 做什麼 | 預期 |
|---|---|---|
| 1 | ⛔ **2026-08-22 起 BLOCKED —— 不是工具問題,是功能在這個 fixture 上根本沒作用**（`[SEETHRUNOOP-2026-08-22]`）。UE 5.4 / DumperTest 上,`FHitResult` 解出來的是一個 **`UStaticMeshComponent`**（連 `bHidden` 欄位都沒有,它的 `bHiddenInGame` 在開/關兩態都是 `false`）,被 `InvokeSetHidden` 自己的 `Actor` 守衛靜靜擋掉。修好回報層（`[SEETHRUTALLY-2026-08-22]`,原本 `hidden_count` 謊報 1）之後,現在誠實地回報 **0**。<br>⚙️ rig 已經寫好:`py tools/verify/seethrough_arms.py run` —— 兩個偵測器,而且 `hidden_count` 沒上升或 DLL 指名的 actor 自己的 bit 沒被設起來時**會拒絕給過**。DLL 現在會在 `seethrough_get_state` 回 `hidden_actors`（`[SEETHRUSET-2026-08-22]`）,所以偵測器②終於能指名道姓。<br>要跑完這一列需要:先修 `[SEETHRUNOOP]`,再換一款真的會隱藏東西的遊戲（docs 記錄 Tower of Mask / DQ7R）。arm (a)(b) 另外需要有人在遊戲裡移動／讓遊戲卡住;arm (c)(d) 已經可以直接跑。 |
