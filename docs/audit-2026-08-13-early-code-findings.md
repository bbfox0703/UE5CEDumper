...
| **U18** ✅ | LOW | `Ubel.cpp` / `Aura.cpp` / `Fern.cpp` (the `// cached — just hash lookup` comments) | **FIXED build 3264 (comments only), and the count had already shrunk to a single true offender.** Three of the four `// cached` sites (`Aura.cpp:8301/8710/9266`) had been migrated to `Ubel::WalkClassEx`, which returns `const ClassInfo&` — so on those the comment understated rather than lied; they now read `// memoized (B10) — BY REF, no copy`, matching the five neighbours already worded that way. The genuine offender is `Ubel.cpp:4935`, still `ClassInfo si = WalkClass(...)` **by value inside a per-field loop** and still captioned "just hash lookup"; its comment now states that a cache HIT is a hash lookup plus a deep copy of the flattened super chain, and says why it is not switched to `WalkClassEx` (the enclosing block is calibrating `DynOff::FSTRUCTPROP_STRUCT`, which `CorrectSubclassOffsets` writes). Its uncommented twin at `:5361` — same shape, same loop — got the same note; the row's Fern.cpp attribution is stale (`Fern.cpp:2124` is a by-ref `WalkClassEx` and carries no comment at all). No behaviour change, per the row's own warning not to make `WalkClass` return a reference. — *As filed:* **NEW 2026-08-17, split out of U5.** Four comments assert a cache hit is *"just a hash lookup"*. It is a hash lookup **plus a deep copy of the flattened super chain** — `WalkClass` returns `ClassInfo` by value, and `Fields` carries the entire inheritance chain (182 entries on a real class is ordinary). Three of the four sit inside **per-value loops**, so the comment is wrong exactly where its reader is deciding whether the call is cheap enough to put in a loop. Fix is the comments plus, where it is easy, hoisting the walk out of the loop. ⚠ Do not "fix" it by making `WalkClass` return a reference — that is what makes bounding the cache legal (U5). | S / low |
...
| **A10** | LOW | `Aura.cpp:2244` / `:3212` (row previously cited `:2990`/`:2235`/`:3203` — stale) | Per-class container/reference metadata caches keyed on a raw `UClass*` the engine recycles, never invalidated. **⛔ DECIDED 2026-08-24 `[A10-SCOPE-2026-08-24]`: NOT IMPLEMENTED, and this is a decision with reasons, not an omission.** Designed and refuted; three reasons, each independently sufficient. **(a) SCOPE — verified, not estimated.** A fix confined to Aura's two caches is a **NO-OP**: both builders read the layout through `Ubel::WalkClassEx` (`CollectContainersRecursive` / `CollectRefMetaRecursive`), which is itself address-keyed (`s_walkClassExCache`, Ubel.cpp:1090) and rebuilds from `WalkClass`, which is address-keyed **and ERASES** (LRU victim, Ubel.cpp:817). So the staleness lives one layer down and the change reaches **five** caches and ~36 lifetime-sensitive conversions across five .cpp files — two of which (`Solide`, `Solitar`) WRITE memory in a live game. **(b) NO COVERAGE.** No test target compiles any of the five (`dll_helpers_test` = `dll_helpers_test.cpp` + `Radar.cpp` + `Denken.cpp`). **(c) THE WITNESS IS PROVABLY PARTIAL.** Whichever fields are chosen it witnesses only the ROOT node, while `CollectContainersRecursive` recurses into nested `UScriptStruct`s and the `ClassInfo` consumed is the FLATTENED super chain — a recycled nested struct, or a changed `ChildProperties` head in a SUPER class, leaves every root word identical and the witness silent. ⛔ And `(InternalIndex, SerialNumber)` is refuted twice over — working-lessons §4.3. **(d) UNFALSIFIABLE TODAY.** Nothing in the tree can recycle a `UClass*` on demand (the C1 spawner does not exist), so no test could be shown to fail. ⚠ **Corrected premise, worth keeping:** `unordered_map` is NODE-BASED, so references survive insert/rehash — the current code is CORRECT today and only an **erase** would dangle. `WalkContainerLeaves` self-recurses while holding its reference and `MatchAddrInStructContainers` does the same, so an LRU here is a use-after-free on ONE thread, no race needed. **REOPEN IF:** a spawner fixture lands; or a session shows Find Refs naming fields of a different class at one address; or the caches are observed growing unbounded. ✅ **A separate, present-tense defect found while scoping it WAS fixed** — see `[A10-EMPTYCACHE-2026-08-24]` in the row below. | M / med |
| **A10b** ✅ | LOW | `Aura.cpp` `GetClassContainers` / `GetClassRefMeta` | Both accessors `emplace` the built metadata **unconditionally**, and nothing ever evicts — so a single transient read fault on `cls` pins an EMPTY result for the whole process and every later scan of that class silently finds nothing. Reachable today: callers guard only `!cls` and pass class pointers read straight out of live objects. **FIXED 2026-08-24 `[A10-EMPTYCACHE-2026-08-24]`:** in the MISS path only, refuse to cache when the class did not walk — `if (Ubel::WalkClassEx(cls).Address != cls) return <static empty>;`. Not a new heuristic: `WalkClassEx` returns the default-constructed `s_emptyClassInfo` (Address 0) both for a null address and when U4's `ShouldPublishClassWalk` refuses (Ubel.cpp:1119), while a good walk sets `info.Address = uclassAddr`. Costs nothing on a healthy class — the builder calls `WalkClassEx` anyway, so it is a cache hit. ⭐ **Regression-controlled LIVE, because the risk was suppressing healthy classes:** DumperTest UE 5.4, same actor, with and without the guard — `classes_primed` **3131 both times**, `objects_scanned` 25,213 both times, same result set. The guard suppresses nothing. ⚠ Not unit-tested: no test target compiles `Aura.cpp` (see A10 (b)). | S / low |
...
| **MB3** ✅ | INFO | `Mimic.cpp:211` | **FIXED build 3268.** Confirmed as filed, and the guard is now the per-iteration form its two siblings already use (`Routine::ReassertLoop` guards per tick, `Stark` per PE call). `RunThreadGuarded` stays as the outer net that stops a throw reaching a frame with no handler; the dispatch `switch` is wrapped in `RunTickGuarded`, so one bad command loses that command rather than the mailbox for the process's life. A caught throw now publishes `MB_ERR_HANDLER_THREW (-11)` + a message instead of leaving `status=PROCESSING` for the script to time out on. ⚠ **Sibling defect found and fixed with it:** `CompoundOpGuard`'s destructor documented itself as covering "any return path, exception included" and published `STATUS_DONE` with whatever `result` held — but the sub-steps only write `result` on failure and `HandleInvokeByName` returns early when it is non-zero, so at the moment a later sub-step throws it is normally **0**. That reported SUCCESS for a command that never completed, and CE Lua reads `result == 0` as "the invoke worked" — strictly worse than the hang. The destructor now compares `std::uncaught_exceptions()` against the count captured at ENTRY (not `!= 0`, so it is correct inside a nested unwind). A new negative result code is not a contract change: the layout and Cmd numbering are unmoved and every script already treats non-zero as failure. — *As filed:* `PollingThreadProc` wraps the **entire** `while (s_running)` loop in `Routine::RunThreadGuarded`, so a throw out of any handler ends the CE mailbox for the session. It is the **only loop in the tree** with the whole-body form — `Routine.h:168-169` (`while (SleepSliced(...)) RunTickGuarded(...)`) and `Stark.cpp:208-210` (guard per PE **call**) both guard per iteration. Filed as INFO, not MEDIUM: see † below. | S / low |
...
| **F5** ✅ | LOW | `Renge.h:282` (+ `Fern::WriteLine`) | **FIXED build 3264 — the filed cost is real, and `merge_patch` was also a CORRECTNESS hazard the row does not mention.** RFC 7386 says a NULL value in the patch DELETES the key rather than setting it, so a handler answering `{"value": null}` shipped a response with no `"value"` at all — and a null colliding with `ok` would have deleted the envelope's own success flag. Worse, a NON-OBJECT payload REPLACES the target outright, taking `id` and `ok` with it. No handler does either today (checked), so this was latent — but it is one plain `data["x"] = nullptr;` away, and silent. Replaced with an `ApplyPayload` helper doing per-key assignment, which has neither behaviour and additionally MOVES out of an rvalue payload; `data` is now taken by value so an lvalue costs exactly what merge_patch cost while `std::move` at the three heavy handlers (`snapshot_chunk`, `find_instances`, `list_all_functions`) costs nothing. Key ORDER is unchanged — nlohmann's default object is a `std::map`, so the serialized bytes are key-sorted either way. `WriteLine`'s `line + "\n"` temporary is gone too: payload and terminator are two `WriteFile` calls on the same `PIPE_TYPE_BYTE` handle under the same `writeMutex`, and short writes are now failures (the old single write ignored `written` entirely, so a truncated response read as success). — *As filed:* `MakeResponse` builds a 3-key envelope then `res.merge_patch(data)`; nlohmann's merge_patch assigns non-object values **by copy**, so a payload a handler carefully built with `std::move` is deep-copied in full — then `WriteLine` materialises a second copy of the serialized string solely to append `"\n"`. Peak ≈ 2× DOM + 2× string, **in the game process's heap**, on `snapshot_chunk` (8192 objects), `find_instances` (50000 cap) and `list_all_functions` (100000 default). | S / low |
...
| **V8** ✅ | LOW | `LiveWalkerViewModel.cs:1348` | **FIXED build 3264 — but NOT where the row points, and the row's own site was already closed.** The `arrayLimit`-truncated TArray/TMap/TSet drill was fixed on 2026-08-18 by `[CONTAINERCAP-2026-08-18]` (commit `3860bfbc`), which added `Core/ContainerTruncation` and wired a "⚠ showing 128 of 199" badge into all six `NavigateTo*Container` / `Populate*ContainerFields` sites. Re-deriving V8 against that tree found the badge sweep had missed **one container path, and it was the worst of the family**: the DataTable RowMap drill. `WalkDataTableRowsAsync` fetches a FIXED page (`limit: 64`) and the UI never pages, yet the crumb printed `dtResult.RowCount` — the TRUE total the DLL reports — so a 5,000-row table announced "5000" over a grid of 64. The other two badge the true total against what they SHOW; this one showed 64 and captioned it 5000. Badged in three places (crumb, header, and the synthetic RowMap preview row the user clicks to drill in) plus a status line, and the status line deliberately uses a NEW `ContainerTruncation.FixedCapStatusLine`: the Array Limit slider does not govern this view, so the existing wording would have layered a second false statement on the first. `BuildContainerLimitWarning`'s "wired into one site" is also stale — it is three (both CE-XML export paths and CSX). — *As filed:* Container drill-down renders an `arrayLimit`-truncated element list (default 128) as the complete container. `BuildContainerLimitWarning` computes exactly this warning and is wired into **one** site: the Copy-CE-XML path. | S / low |
| **V9** ✅ | LOW | `ObjectTreeViewModel.cs:426` (`SearchAsync`) | **FIXED build 3264.** Confirmed exactly as filed, including that the Cancel button really is the only enabled control: `ObjectTreePanel.axaml:53-54` binds `CancelLoadCommand` with `IsVisible={Binding IsLoading}` while Search/Reload/the filter box are all `!IsLoading`. Fixed by copying `LoadAsync`'s own shape ten lines above — replace `_loadCts`, take a token, pass it to `SearchObjectsAsync`, add the `gen != _loadGen` supersede guard after the await, and catch `OperationCanceledException` separately so a user cancel reports "Search cancelled" and leaves the previous (still valid) result set on screen instead of blanking it as an error. Same family as X6 (L6) and AE20 (L9). — *As filed:* `SearchObjectsAsync` is called without the `CancellationToken` it accepts, and `CancelLoadCommand` cancels `_loadCts` — which `SearchAsync` itself cancelled at `:411` and never replaced. For the whole search the panel's only enabled control is a Cancel button that structurally cannot do anything. | S / low |
| **V10** ✅ | LOW | `PointerPanelViewModel.cs:551` (`Update`) | **FIXED build 3264, and the user-visible consequence is bigger than the one filed.** The filed race (ComboBox gated on `IsApplyingOverride`, so a mid-scan override apply clears `IsScanning` and re-enables `CanExtraScan`) is real, but the ORDINARY path is worse: `ExtraScanAsync`'s success branch sets `ScanResultText = "Found: GObjects: 0x…"` and then raises `RescanApplied`, whose MainWindowVM handler awaits `GetPointersAsync` and calls `Update` — which blanked that result milliseconds after the user saw it. So the Extra Scan erased its OWN result, every time it succeeded, with no race needed. `ApplyInvokeTimeoutAsync` reaches the same code from a pure settings round-trip. Fixed by moving the five assignments out of `Update` into a new `ResetScanState()` owned by the FRESH-SESSION paths only (`ClearOnDisconnect` + MainWindowVM's `ApplyEngineState`) — the scan commands already own those properties end to end. — *As filed:* `Update(EngineState)` unconditionally resets `IsScanning`/`ScanComplete`/`ScanStatusText`, but `ExtraScanAsync` owns `IsScanning` across its 1.5 s polling loop and clears it in its own `finally`. `Update` is reachable mid-scan via the fire-and-forget `ApplyOverrideAsync`, whose ComboBox is gated on `IsApplyingOverride`, not `IsScanning`. | S / low |
| **V11** ✅ | LOW | `PointerPanelViewModel.cs:988` | **FIXED build 3264 — two sites, not one.** Confirmed as filed; the sibling grep found `RegisterGEngineSymbolAsync` doing the same thing 30 lines below `RegisterGWorldSymbolAsync`. Both now route through one `ReportSymbolRegistration`: success sets a new bound `SymbolStatusText` (teal, next to the `ErrorMessage` banner V7 added), failure goes to `ErrorMessage` itself, which `PointerPanel.axaml` already renders in red. ⚠ **The fix needed a guard against re-introducing itself**: `Res.Get`/`Res.Format` return an EMPTY STRING for a key they cannot resolve (no throw, no log — the trap `SelfTestAdviceTests.EveryAdviceKeyExistsInEnAxaml` exists for), and both new bindings are gated on `IsNotNullOrEmpty`, so a typo'd key would have rendered as no message at all — V11 arriving by a second route. A short literal fallback keeps the report visible, and a test pins both en.axaml keys. — *As filed:* `CreateSymbolScriptAsync`'s `bool` is branched on only to pick `_log.Info` vs `_log.Warn`. Neither branch touches a bound property, so "Register symbol" looks identical whether CE registered it or the bridge never reached CE. The sibling call site in `LiveWalkerViewModel` does this correctly. | S / low |
...
| **W8** ✅ | LOW | `UsmapExportService.cs:90` | **FIXED build 3264.** Confirmed as filed and as the LAST unfixed mirror of a rule the tree already states in three places: `Aura::IsClassLikeMeta` (DLL, build 673), `DumpAllService.ClassLikeMetas`, and `SdkExportService`, whose comment names this exact bare-`"Class"` check. Dropped meta-classes were `BlueprintGeneratedClass`, `AnimBlueprintGeneratedClass`, `WidgetBlueprintGeneratedClass` and `DynamicClass` — i.e. every Blueprint class, which is most of what a `.usmap` consumer needs to parse saved games and network traffic. One-line fix routing through `DumpAllService.IsClassLikeMetaName`; the test asserts against that shared predicate rather than a second hand-written list, so the two exporters cannot drift apart again. Repo-wide grep confirms no third site. — *As filed:* The class collector accepts only `ClassName is "Class" or "ScriptStruct"`, so every `BlueprintGeneratedClass` / `AnimBlueprintGeneratedClass` / `WidgetBlueprintGeneratedClass` is silently dropped — on a normal shipped title that is thousands of classes vs a few hundred native ones. Both sibling exporters use the whitelist predicate, and `SdkExportService` carries a comment naming this exact bare-`"Class"` check as a bug fixed in DLL build 673. | S / low |
...
| **Y10** ✅ | LOW | `BakedScriptGenerator.cs:223` | **FIXED build 3264 — and the pre-zero loop was UNBOUNDED, which the row does not say.** The missing `AppendContractCheck` is confirmed and fixed (it now precedes the first `writeByte`, with `UntickAndReturn` so a bail that applied nothing does not leave a ticked record). Reading the loop for the check's placement exposed the second half: it zeroed `parmsSize` bytes at `mb + 0x328`, and `parmsSize` is `UFunction::ParmsSize`, which the mailbox does not bound. `paramsData` is `uint8[1024]` followed immediately by `cmdFlags`/`cmdOutFlags`, and `MailboxData` ends at 0x730 — so a UFunction with a large by-value struct parameter zeroed through both flag words and past the end of the struct. The Lua helper's own `invokeUFunction` *does* refuse `parmsSize > 1024`, but it runs AFTER this loop, so the refusal came too late to prevent the write. Now clamped to `ParamsRegionBytes`. ⚠ **No contract move**: both changes are script-side, nothing about the layout changed, and `tools/check_mailbox_contract.py` passes unchanged at contract 3 / min 1. — *As filed:* Verify mode writes into the mailbox (`writeByte(_PD_dbg + i, 0)` over `parmsSize`) **before any contract check** — `BakedScriptGenerator` is the only mailbox-touching generator with no `AppendContractCheck`, and CLAUDE.md's rule is explicit that the check comes *before the first write* because the layout is what is in question. | S / low |
| **Y11** ✅ | LOW | `ParamBufferBuilder.cs:228` | **FIXED build 3264 — and the honest answer differs by type, which the row's single sentence hides.** Confirmed: `WriteParam`'s size-driven default wrote the textbox as a raw int32 over the structure's first pointer field. But the exported-script half of the same dialog had ALREADY been fixed (audit #5 AA16, in `writeParams`) and it does **not** treat these types alike — so copying one rule for all of them would have made FIRE disagree with the baked script in the other direction. Two predicates instead: `IsEmptyOnlyParam` (TArray/TMap/TSet/delegates/ TFieldPath/TOptional/layout-less StructProperty) leaves the slot ZEROED, which is the default-constructed empty value for each, and refuses only a value the user actually typed; `IsRefusedParam` (FText alone) refuses unconditionally, because an all-zero FText holds a `TSharedRef` the engine dereferences and is a crash rather than a default — exactly what the helper's `ftext` arm already says. `StructProperty` is in the first set for the LAYOUT-LESS case only; an expanded struct still writes through `WriteStructParam`. — *As filed:* FIRE has no unsupported-param-type gate: an `FText`/`TArray`/`TMap` param's textbox is written as a raw int32 into the struct's pointer field. | M / low |
| **Y12** ✅ | LOW | `InvokeParamDialog.cs:856` | **FIXED build 3264.** Confirmed as filed. The clipboard fallback now wraps through `CheatTableBuilder.WrapAaScriptXml`, matching all eleven sibling call sites, and the messages say "copied as CE XML — paste into CE's address list" rather than implying a bare body can be pasted into a memory record. Picked up in passing: `CopyToClipboardAsync` became `Task<bool>`-returning-and-never-throwing on 2026-08-19 (`dcafa5fe`) and this call site ignored the result, so a clipboard the app could not write was still reported as a successful copy — the same defect as Y14 one branch up, and now reported as an error. — *As filed:* The baked-invoke clipboard fallback still copies a raw AA body — the build-1986 `WrapAaScriptXml` sweep reached the no-arg sibling **two lines away** and not this one. | S / low |
| **Y13** ✅ | LOW | `BakedScriptGenerator.cs:195` | **FIXED build 3264.** Confirmed, and the case is more common than "edge": the return slot is the LAST param, so any function with two 8-byte inputs plus an `FString` return already clears the flat 32-byte window the next printed line told the user to read the value out of. `ComputeDumpLength` now grows the window to `returnParam.Offset + Size`, then clamps to the function's own `parmsSize` (reading past it would render mailbox fields as params), to a 256-byte ceiling, and to the 8-byte floor the old expression had. When the clamps still win, `ReturnFitsInDump` swaps the "see After: dump above" hint for one naming the offset and pointing at CE's memory viewer — the point of that line is that the bytes are in the text above it, so it must not claim so when they are not. — *As filed:* Verify mode's 32-byte dump window cannot contain the complex return it tells the user to read. | S / low |
| **Y14** ✅ | LOW | `InvokeParamDialog.cs:850` | **FIXED build 3264.** Confirmed as filed — the canonical *"the report and the reality are computed by different code paths"* shape this audit keeps finding. The export message now names the params that failed and turns amber, on all three delivery branches. The detection is `BakedScriptGenerator.IsUnparsedLiteral`, which decides by RUNNING the real `RenderLiteral` and testing for the marker it stamps — deliberately not a second parse, which is how the two paths would drift apart again; the `MarkUnparsed` prefix is now a shared constant so the predicate and the emitter cannot disagree about it. — *As filed:* *"AA Script created in CE … (N baked param(s))"* is reported even when a param failed to parse and was baked as 0. | M / low |
...
| **AF7** ✅ | LOW | `Denken.h:37` (Denken::NativeAnalysisResult::budgetHit) | **FIXED build 3263.** Confirmed. `budgetHit` now travels `Denken -> Aura::FunctionPropRefResult.budgetHit -> Fern "budget_hit" -> FunctionPropRefsResult.BudgetHit -> UI`. **Two consumers, not one:** the Props dialog gains `PartialResultNotice.DisassemblyBudget()` and turns amber, and the sibling grep found `InterestingFunctionsViewModel`'s batch **Uses** cell writing a bare `0` for a budget-truncated scan — the Z9 shape exactly, now `0 ⚠ partial` plus a roll-up clause. `BatchPartialClause` gained a `cause` parameter (defaulting to today's wording) rather than reusing "hit the scan deadline", which would have named the wrong cause in the one class whose stated rule is to name the cause. Set BEFORE the mapping loop so an early return cannot drop it again. — *As filed:* Path-2 native disasm's "result may be partial" flag is written, logged, and then dropped before the wire — the Xref dialog shows a truncated field list as complete | S / low |
| **AF8** ✅ | LOW | `Solide.cpp:128` (Solide::ReadNumeric (byte-width fallback)) | **FIXED build 3263.** Confirmed, and the mechanism is the READ, not the write as filed: `WriteNumeric` stored `static_cast<uint8_t>(-5)` = 0xFB, which IS -5 in an int8, but `ReadNumeric` read it back unsigned as **251** — so the drift check `read != target` could never be satisfied and the worker rewrote the same byte forever. The mirror case is worse: forcing 200 stored 0xC8, the game saw **-56**, and the unsigned read agreed with the target so the hold looked converged. Fixed by deriving width AND signedness from one table, `Solide::IntWidthOf`, placed **header-inline** because no test target compiles `Solide.cpp`; `WriteNumeric` now also REFUSES an out-of-range target instead of truncating into it (the same non-convergence by another route). 20 assertions in `dll_helpers_test`. — *As filed:* Int8Property is READ as unsigned while it is WRITTEN as signed, so a negative forced value never converges and the re-assert worker reports permanent drift against the game | S / low |
| **AF9** ✅ | LOW | `Constants.cs:37` (Constants.MaxProcessFolders) | **FIXED build 3263.** Confirmed and **worse than filed**. Three things, any one sufficient: CLAUDE.md's Log-output rule says retention is by AGE and explains why a count cannot express it, and `Constants.cs` said the same **four lines above** `MaxProcessFolders = 20`; there was nothing to guard against, because a folder is named after the GAME (`SanitizeFolderName(processName)`), not the PID, so the count is "distinct games played in 21 days" and cannot run away; and the eviction was `Delete(recursive: true)` on a folder the **DLL also writes**, whose own sweep (`Sein.cpp::PruneStaleProcessFolders`, comment: *"Age-based, matching the file policy"*) had deliberately kept it. So the UI silently destroyed the DLL's five log categories inside the window. `CleanupProcessFolders` / `SelectFoldersToEvict` / `MaxProcessFolders` removed; `CleanupOldLogFolders(LogMaxAgeDays)` is the sole owner. The five B37 tests pinned the CAP's ranking and are replaced by three that drive the real service against real directories (30 in-window folders survive; a >21-day one still dies; the UI folder stays). — *As filed:* A COUNT cap silently deletes whole per-game log folders inside the 21-day window that CLAUDE.md says is the only retention rule | S / low |
| **AF10** ✅ | LOW | `Program.cs:27` (Program.Main) | **FIXED build 3263.** Confirmed against current code (`Program.cs` was edited twice the same day). `StartWithClassicDesktopLifetime` RETURNS the lifetime's exit code and `Main` discarded it for a hardcoded `return 0`, so `App.axaml.cs`'s deliberate `desktop.Shutdown(1)` on the second-instance path reported success to anything scripting the exe. One line. Sibling grep: this is the only `StartWith*` call in the tree. — *As filed:* Main discards the process exit code, so the deliberate Shutdown(1) on the second-instance path reports success | S / low |
| **AF11** ✅ | LOW | `CoordinateLibraryStore.cs:43` (CoordinateLibraryStore..ctor) | **FIXED build 3263.** Confirmed — `_dir` was `%LOCALAPPDATA%\UE5CEDumper` itself, and the family is unbounded per game and **four files wide** (`.json` + `.bak` + `.preimport.bak` + `.preclear.bak`). Now `AppDataFolderMaintenance.Prepare(root, "TeleportCoords", prefix, maxAgeDays: 0, log)` **from the constructor**, matching `BookmarkStore` exactly. All four names share one `GameKeyOf` key, so the group-move invariant applies unchanged. **Retention side chosen and justified:** sweep OFF. The rule's own distinction is regenerable-capture (`Snapshots\`, swept) vs hand-placed-unrecoverable (`Bookmarks\`, not) — a coordinate library is the second and then some: hand-authored, labelled, importable from a paste, and this store already keeps three backup flavours precisely because losing it is the worst outcome in the feature. — *As filed:* A third unbounded per-game file family writes to the app-data ROOT, bypassing AppDataFolderMaintenance -- and Constants.cs asserts it does not exist **[2 lenses]** | M / low |
| **AF12** ✅ | LOW | `GroupMatch.cs:285` (GroupMatch.Run) | **FIXED build 3263 (direction call: the COMMENT was wrong, not the code).** Confirmed: `ValueSearchViewModel.GroupPerSlotCap` is a user-settable 8..4096 picker sent to the DLL as `per_slot_cap`, while both snapshot call sites take the compile-time default — so the comment's guarantee (*"two different caps would make the same object match on one page and not the other"*) is exactly what happens once the user touches the picker. **Coupling them was rejected**: wiring a snapshot query to another panel's combo box is a surprising product decision, not a bug fix (§2.3's "a capability gap is not a corruption"). What the code owed instead was honesty — the comment now says the shared thing is the DEFAULT and names the divergence, and `SnapshotGroupResult.PerSlotCap` reports the cap actually applied. — *As filed:* The "shared per-slot cap" invariant the comment insists on is broken the moment the user changes the live cap | S / low |
| **AF13** ✅ | LOW | `GroupMatch.cs:302` (GroupMatch.Run (per-slot cap truncation)) | **FIXED build 3263.** Confirmed. `GroupMatch.Run` gained a **required** `out bool perSlotCapHit` (§2.2 — an optional one is how the next call site drops it), surfaced as `SnapshotGroupResult.PerSlotCapHit` and rendered with the EXISTING `PartialResultNotice.PerSlotWitnessCap` the live path has used since AE13. Two design points: the flag is set **independently of the return value**, because a slot truncated at the cap that then fails the assignment is precisely when a MISS may be an artifact — pinned by its own test; and `SpcGroupResult` deliberately did NOT get the flag, because `SpcGroupQueryAsync` runs its own inline matcher with no per-slot cap at all, so there is nothing to truncate. 28 test call sites updated. — *As filed:* Snapshot group match truncates a slot at 256 leaves with no signal of any kind — the DLL sibling at least logs it | S / low |
| **AF14** ✅ | LOW | `MovementScriptGenerator.cs:163` (MovementScriptGenerator.EmitGravDirBlock) | **FIXED build 3263 — NOT a contract change, and proven so.** The literals were numerically CORRECT (`0x328 + 8` / `+ 0x10`); the defect is that one statement group wrote its first operand through `OffParamsData` and the next two as bare constants, so a future move of `params_data` would have split an FVector across the old and new layouts. Sibling grep found it **under-applied**: `TeleportScriptGenerator` had six more (coord X/Y/Z, trace channel, mode, and two `readDouble` triples that hardcoded `0x328` as well). All now go through new `OffParamsData1`/`OffParamsData2`. ⚠ Two of those lines were NON-interpolated strings — substituting naively would have emitted literal `{CeMailboxLayout...}` braces into the Lua; caught and fixed. The emitted text is byte-identical, which the generators' existing literal assertions (`"writeDouble(mb + 0x330,"`) already pin, plus a new arithmetic invariant test. `check_mailbox_contract.py`: **OK, v3 (min 1), surface hash matches**. — *As filed:* Gravity-direction emitter writes X through CeMailboxLayout.OffParamsData but Y and Z at raw 0x330 / 0x338 in the same statement group | S / low |
| **AF15** ✅ | LOW | `TeleportViewModel.cs:3914` (TeleportViewModel.PushCoordLuaNoDllAsync / Sav) | **FIXED build 3263.** Confirmed: 1 of 3 export paths built the disclosure, the no-DLL push and "Save .lua" both discarded it with `out _`. Extracted to one `FoldedGroupsNote(folded)` used by all three rather than copied twice — three inline copies of a disclosure is how the wording drifts until two of them are wrong. Returns "" when nothing folded, so callers append unconditionally (the `PartialResultNotice` shape). — *As filed:* The coordinate-library "N group(s) had no radio button" disclosure is emitted at 1 of the 3 export call sites; the other two discard it with `out _` | S / low |
| **AF16** ✅ | LOW | `DetectStatsPanel.axaml.cs:14` (DetectStatsPanel..ctor) | **FIXED build 3263 — and the sweep it prompted found FOUR MORE SITES.** See the AF16-AF23 rollup below the table. — *As filed:* Four panels break the AOT sort rule their sibling panels' comments spell out — column headers are clickable and do nothing in the shipped trimmed build | M / low |
| **AF17** ✅ | LOW | `DetectStatsPanel.axaml.cs:14` (DetectStatsPanel..ctor) | **FIXED build 3263.** Exactly as filed: `DetectStatsPanel` made no `WireSortComparers` call, and 2 of its 7 sortable columns (`IsConfirmed` rendered as `ConfirmBadge`, `PropOffset` rendered as `OffsetHex`) sort on a path no binding roots. The other five bind and sort on the same path and are rooted. `PropOffset` is wired to the NUMERIC property, not the hex string (§3.2 rule 1). — *As filed:* Detect Stats panel makes no WireSortComparers call at all; two of its seven sortable columns sort on a path no binding roots | S / low |
| **AF18** ✅ | LOW | `FunctionPropsDialog.cs:165` (FunctionPropsDialog.BuildUi (_grid)) | **FIXED build 3263.** Six `DataGridTemplateColumn`s, `CanUserSortColumns = true`, no comparer — a code-built grid is not exempt from the rule the XAML panels follow. Wired through the same `WireSortComparers` helper. — *As filed:* Code-built xref grid enables sorting on six template columns with no CustomSortComparer — every header is dead under AOT | S / low |
| **AF19** ✅ | LOW | `LiveFuncsPanel.axaml.cs:15` (LiveFuncsPanel.ResultsSortComparers) | **FIXED build 3263.** `MeanPeriodMs` added; the column renders `PeriodLabel`, so nothing rooted the sort path — on the single column the Phase E cadence feature exists for. Its file comment claimed "the text columns sort out-of-box", which is true only when SortMemberPath equals the Binding path; corrected. — *As filed:* Live Funcs comparer dictionary omits MeanPeriodMs, leaving the Period column — the point of the Phase E cadence feature — unsortable under AOT | S / low |
| **AF20** ✅ | LOW | `LiveWalkerPanel.axaml.cs:44` (LiveWalkerPanel (ctor) / WireSortComparers) | **FIXED build 3263.** Confirmed, and subtler than a template column: **Params** IS a `DataGridTextColumn`, but its value comes from a `<MultiBinding>` in ELEMENT syntax — a reflection binding, which does not root `NumParms`. That is why it read as safe. The file's third grid was checked and needs nothing (every column binds and sorts on the same path). — *As filed:* AOT sort comparers are wired onto 1 of the file's 3 DataGrids; FunctionGrid's "Params" column breaks the repo's own documented AOT sort rule and its header is a silent no-op in the shipped binary **[2 lenses]** | S / low |
| **AF21** ✅ | LOW | `MainWindow.axaml.cs:276` (IsSnapshotPositionAcceptable) | **FIXED build 3263.** Confirmed unit mismatch: `pos` is a `PixelPoint` and the working areas are physical px, but `_pendingWidth`/`_pendingHeight` are Avalonia DIPs — at 150% a 1400-DIP window was measured as 1400px instead of 2100px. **Converted the SIZE side**, because that is the side in the wrong unit; the position and screens are natively physical and converting them would change the helper's whole contract. This is verbatim what the sibling guard one method up already does with `_normalWidth * RenderScaling`. Sibling grep: the other two call sites (`OnOpenedRestoreCheck`, `CenterIn`) already convert — AF21 really was the one missed site. — *As filed:* Window-placement guard feeds DIP sizes to a helper documented as taking physical pixels, so on a HiDPI monitor a legitimately-placed window is rejected and its position stops being tracked | S / low |
| **AF22** ✅ | LOW | `PropertySearchPanel.axaml.cs:57` (PromptForceValueAsync) | **FIXED build 3263.** Confirmed, and it is more than the button: the dialog was titled *"Freeze property value"*, labelled the field *"Freeze value"*, and its inherited-field caveat advised *"edit className in the generated CFG block"* — advice with no CFG block to edit on a path that generates no script (the Z10 unreachable-advice trap). Added `FreezeValueDialog.Purpose { Freeze, Force }` with all four strings in `en.axaml` via `Res`. `ScopeWarning`'s hint parameter is **required, no default**: the existing test's own words are *"it must say how to narrow it, or it is only an apology"*, and a default is how a new call site would drop it again. Missing-resource fallback is the KEY, not the Freeze wording — visibly wrong beats plausibly wrong. — *As filed:* The "Force value…" flow shows the Freeze dialog verbatim, so the button the user clicks says "Create freeze script" while the action writes and holds the field on every live instance | S / low |
...
> ### 🔍 AF16–AF23 — six findings, one defect, and the sweep found ten sites (build 3263)
>
> The six AOT-sort rows are all *"this grid was never wired"*, so they were fixed as one pass and
> then the whole tree was swept rather than trusting the finder's list. **34 DataGrids, 162
> user-sortable columns, 30 of them unsafe across 10 sites.** The findings named six; the sweep
> found four more, which is the point — closing six of ten is how this recurs.
>
> **The rule, stated precisely enough to machine-check** (it is in
> `Helpers/DataGridSortComparers.cs` prose, but never as a predicate): a user-sortable column is
> safe iff **either** its own `Binding` path equals its `SortMemberPath` — the compiled binding
> roots the property, so the reflection sort still resolves it — **or** a comparer is wired for
> that path. 132 of the 162 columns pass on the first arm, which is why the omission is easy to
> make and impossible to see.
>
> **The four unfiled sites:** `ClassPivotPanel` (5 columns), `SnapshotPanel` (4), `SpcPanel` (1),
> and `ObjectInstancePickerDialog` (4). Three of them had no `x:Name` on the grid, so they could
> not have been wired without also naming them — a small structural reason the defect clustered.
>
> **The seventh site carried a written justification for the defect**, which is how it survived
> (§2.2's "expect the siblings to have TESTS defending them", in comment form):
> *"SortMemberPath is kept so sortable headers still work in the non-AOT default build; sort is a
> runtime nice-to-have"*. True, and beside the point — the build we SHIP is the trimmed one, so the
> headers were dead for every user. Read such a comment as evidence about the belief, then check
> the belief's premise.
>
> **Two false positives, both instructive.** `ValueSearchPanel` looks unsafe and is not: it
> attaches a `Sorting` handler that sets `e.Handled = true` and drives a server-side re-sort of the
> whole windowed result set, so a comparer there would be dead code. And three `Offset` columns
> bound `{Binding Offset, StringFormat=0x{0:X}}` — the path still roots the property; the format is
> applied afterwards. That second one was caught by **two independent detectors disagreeing**
> (a Python sweep and the C# guard), which is §1.4 doing its job: the guard's regex was wrong, not
> the code.
>
> **Structural guard shipped** — `DataGridSortWiringTests`, two halves, because the code-built
> dialogs are invisible to a XAML parse and all three of them shipped unwired: one parses every
> `Views/*.axaml` and applies the rule above, one scans `Views/*.cs` for
> `SortMemberPath = nameof(T.P)` and demands a matching comparer key in the same file. Both were
> **negative-controlled** (remove `["MeanPeriodMs"]` → the XAML half fails naming that exact
> column; remove `["Kind"]` → the code half fails naming that exact path), and both carry a
> "guard the guard" floor assertion so a parse that silently matches nothing fails loudly instead
> of passing vacuously.
>
> ⚠ **What the guard cannot do, stated so nobody assumes otherwise:** it checks the pairing of
> markup to comparer table, not the runtime behaviour. The defect only manifests in a trimmed
> binary and a test cannot trim itself, so the AOT spot-check rows in todo.md's verification
> register remain necessary.
...
| **AD23** ✅ | INFO | `Himmel.h:156` (file-header UE 5.8 note ("back into a base ancho) | **FIXED build 3268 (docs).** Direction: the HEADER was wrong, the entries right — and the finding was the SMALL half of its own defect. Only `-0x10` (V10/AV1/AV2/RE2/V12) produces a BASE anchor; `-0x14` (DI427_3/G427_2) and `+0x0C` (G427_4) both land on **ObjObjects**, which DI427_3's own comment states and `ValidateGObjects`'s "Default" preset independently confirms (it reads Num at +0x14 from the address given, so NumElements-0x14 IS ObjObjects). That also corrects the paragraph's 5.8 conclusion: `-0x14` does **not** overshoot on 5.8, because its anchor and target are both INSIDE ObjObjects and the reorder moves them together. `-0x10` and `+0x0C` do overshoot, as stated. — *As filed:* The header says the -0x14 and +0x0C adjustments produce an FUObjectArray base anchor; the entries themselves say they land on ObjObjects | S / low |
| **AD24** ✅ | INFO | `Renge.h:268` (Renge::MakeResponse / MakeError / MakeEvent / Ad) | **FIXED build 3268.** ⚠ **The finding was RIGHT and my re-derivation was wrong — the linker settled it.** I first refuted "structurally UNLINKABLE" on the grounds that everything in Renge.h is `inline`, wrote the test, and got LNK2019: `MakeResponse` calls `Stark::IsGameThreadResponsive`, DECLARED in Stark.h but DEFINED in the uncompiled `Stark.cpp`. `inline` makes the builder linkable and does nothing for what it calls. Closed by supplying that one dependency as a stub in the test TU (rather than dragging MinHook in via Stark.cpp) — it must match the declaration Renge.h already pulls in, so a signature change is a compile error, not a silently diverging fixture. `Test_Renge_EnvelopeBuilders`, 19 assertions over MakeResponse / MakeError / MakeEvent: envelope keys, the documented `game_thread_stalled` precedence, MakeError's exact 3 keys, MakeEvent's absent `id`, and the non-object-payload case for both. `game_thread_stalled`'s VALUE is never asserted (the stub owns it) — only presence and type. — *As filed:* Enumerated coverage gaps in the in-scope headers — including the pipe response envelope, which is structurally UNLINKABLE from the only test target that includes it | M / low |
| **AD25** ✅ | INFO | `Utf8Helpers.h:236` (IsWellFormedUtf8) | **FIXED build 3268.** Direction: the CODE. `hasMultiByte` is now accumulated into a local and committed at the single success exit, so it can no longer contradict a `false` return. No live bug existed — `DecodeFStringBuffer` gates on `utf8Ok`, which short-circuits — so this removes a latent trap rather than a fault. 4 new assertions; **negative-controlled**: restoring the eager `hasMultiByte = true` turns the suite red at `utf8_helpers_test.cpp:595` (`Pass: 257  Fail: 1`) on exactly the one discriminating case, and the flag is seeded `true` first so "left untouched" fails the same as "wrongly set". — *As filed:* hasMultiByte is left true when the function returns false, and nothing documents that the out-param is only meaningful on a true return | S / low |
| **AD26** ✅ | INFO | `utf8_helpers_test.cpp:204` (Test_EncodeUtf16_OutputAlwaysValidUtf8 / Test_De) | **FIXED build 3268.** Direction: the INPUT, not the comment — the test could not reach the case it named. `'A', 0xD800, 0xDE00` under "lone high + lone low" is a **valid surrogate pair** (0xDE00 is a LOW surrogate), which EncodeUtf16 combines into U+10200; the array's only lone surrogate was the trailing 0xDC00, so the lone-HIGH branch was never executed. Input now carries `0xD800, 'B'` (high with a non-low successor) alongside the valid pair, plus an exact-bytes assertion — Sanitize-stability alone would still pass if a lone surrogate were dropped instead of replaced. — *As filed:* Two test comments describe coverage the tests do not provide — one input is not the pathological case it is named for, the other's deliberate setup is inert | S / low |
| **AD27** ✅ | INFO | `utf8_helpers_test.cpp:358` (Test_Decode_Utf8CjkWithTrailingHeapBytes) | **FIXED build 3268.** Direction: the COMMENT. The claim that a NUL pair at [66,67] would make the UTF-16 hypothesis false-fire is contradicted by `Test_Decode_Utf8MultiByteBeatsZeroHeapTail` 80 lines below, whose whole point is that Rule 2 (`utf8Ok && utf8HasMultiByte`) returns BEFORE the UTF-16 hypothesis is evaluated. Claim dropped; the two bytes are kept as realistic dump data and the removed sentence is replaced by a **negative control** that zeroes exactly that pair and asserts the same answer. Two tests in one file disagreeing about one rule was the real defect. — *As filed:* The test's deliberately-non-zero UTF-16 terminator bytes are unreachable setup, and its comment is contradicted by the test 80 lines below | S / low |
...
| **AC13** ✅ | INFO | `PipeClient.cs:224` (PipeClient.SendAsync) | **FIXED build 3268.** Direction: the CODE — the comment stated the intent correctly ("a cancelled or faulted request still consumed transport time") and the placement defeated it. `SendAsync` is now a thin timing wrapper around a new `SendCoreAsync`, so the write-lock wait, `WriteLineAsync` and every throw out of the IOException/ObjectDisposedException classifier are inside the measurement. A misbehaving pipe is exactly when writes fail, so the dropped samples were the ones most worth having. The not-connected guard deliberately stays ABOVE the timer — nothing was sent, and a 0 ms sample would deflate the average as surely as the gap inflated it. — *As filed:* PipeTransportStats.Record sits in a finally that wraps only the response await, so a request failing in the WRITE contributes zero transport time — precisely the case its own comment says must be counted | S / low |
| **AC14** ✅ | INFO | `PipeClient.cs:240` (PipeClient.ReadLoopAsync) | **FIXED build 3268.** Direction: the CODE. `_reader` is captured into a local before the loop, exactly as `SendAsync` captures `_writer` and for the same documented reason. `Dispose()` calls `CloseStreams()` — which nulls the field — WITHOUT awaiting the read loop, so the null can land between the `!= null` test and the dereference; the resulting NRE misses both catch filters and logs an ordinary shutdown as "ReadLoop error". A disposed local throws ObjectDisposedException, which IS filtered. Capturing once is sound: `_reader` is assigned in `ConnectAsync` before the task starts and is only ever nulled, never replaced. Sibling-grepped: every other `_reader`/`_writer`/`_pipe` use is `?.` or the ctor assignment. — *As filed:* ReadLoopAsync null-checks `_reader` as a field and then dereferences the field, the exact pattern SendAsync captures into a local at :140 to avoid — an NRE the loop's IOException/ObjectDisposedException catches do not cover | S / low |
| **AC15** ✅ | INFO | `ProxyDeployService.cs:420` (ProxyDeployService.TryDetectUeVersion) | **FIXED build 3268.** Confirmed on both halves. `FileVersionInfo.GetVersionInfo(exePath)` — a full VERSIONINFO resource load per detected game, on every Steam scan and every generic drive walk — was read into a local and then unconditionally discarded, and `DetectedGame.UeVersion` is read by nothing in the app. The dead load is gone; the method stays as a documented seam returning null (the DLL detects the version at run time), which keeps `ProxyDeployTests`' existing `Assert.Null(game.UeVersion)` as the regression guard. — *As filed:* `TryDetectUeVersion` reads every game exe's full VERSIONINFO resource and unconditionally discards it; the property it feeds is never read by anything **[2 lenses]** | S / low |
| **AC16** ✅ | INFO | `ProxyDeployService.cs:1608` (ProxyDeployService (leftover-proxy cleanup region)) | **RE-VERIFIED 2026-08-19 — NEGATIVE RESULT STILL HOLDS, no code change.** Re-checked against current code because `ProxyDeployService.cs` was rewritten the same day (`9ef5b8ca`, audit L7). All three invariants hold: (1) `:1797` `MoveToRecycleBin` is the sole disposal in the region and its failure branch records `failed` rather than falling back to a hard delete; (2) `:1842` `Directory.Delete(dir, recursive: false)` deepest-first, `catch (IOException)` breaks the ascent, and repo-wide there is **zero** `Directory.Delete(x, true)` in production code; (3) `ProxyOrphanScanner.cs` has exactly two usings (`System.Text`, `UE5DumpUI.Models`) and every `Path.`/`File.`/`Directory.` token in it is inside a comment. Noted for the future: `UndeployAsync` at `:1260` DOES hard-delete a proxy path — correct under the policy split, but 35 lines above the region boundary and the likeliest place for a later edit to blur the two. — *As filed:* NEGATIVE RESULT — CLAUDE.md's three leftover-cleanup invariants all hold as written; do not re-audit this surface without new evidence | S / low |
| **AC17** ✅ | INFO | `WindowsPlatformService.cs:790` (WindowsPlatformService.VolumeHasRecycleBin) | **FIXED build 3268.** Direction: the CODE, and the pre-filter was silently undoing the paragraph directly above it. `DriveInfo`'s constructor normalizes through `Path.GetPathRoot`, so for the mount-point root this method works to obtain (`C:\Mount\Games\`) it reported on `C:\` — the HOST volume, the exact wrong-disk answer `GetVolumePathName` is called to avoid. Since a mount point's host is essentially always Fixed, the filter degenerated to "always pass" for precisely the paths it was meant to judge. Now `GetDriveTypeW(root)`, which takes the path as given. ⚠ **Sibling sites found and NOT changed** (different features, live-only to verify): `WindowsPlatformService.GetFreeDiskSpaceBytes` `:407` and `GetTotalDiskSpaceBytes` `:419`, and `WindowsLogCompressionService.IsSupported` `:71`, all `Path.GetPathRoot` + `DriveInfo` — same wrong-volume shape. Tracked in todo.md. — *As filed:* The fixed-drive pre-filter on the Recycle-Bin gate answers about the HOST volume — `new DriveInfo(root)` re-runs the exact `Path.GetPathRoot` lookup the comment three lines above says it avoids | S / low |
...
| **AB9** ✅ | LOW | `Heiter.cpp` (DllMain → Sein::Init / Sein::InitProcessMirror) | DllMain did shell32 + unbounded recursive filesystem work under the loader lock. **FIXED 2026-08-24 `[AB9-SWEEP-2026-08-24]`:** the retention sweep is now `Sein::RunRetentionSweep()`, called by DllMain — on a thread where threads are allowed (AFTER the module PIN), **INLINE in a CE host** because AB1 forbids a thread there (the module is deliberately not pinned, and CE FreeLibrary's its plugins). ⭐ **Measured live, not argued:** the DllMain window (`DLL loaded` → `auto-start thread created`) went from a **138 ms median across 407 real sessions** (3,023-file / 32-folder corpus) to **16 ms**, with the sweep itself logging **219 ms** on the deferred thread afterwards — and 16 ms is essentially the 15 ms floor those 407 sessions showed on a small tree, so the excess really was the sweep. ⚠ Re-derived, correcting the row as filed: `Sein::Init` is NOT the heavy half (`SHGetKnownFolderPath` + `create_directories` only); everything unbounded was the last **two statements** of `InitProcessMirror`, which is a clean seam. ⚠ The obvious implementation SELF-DEADLOCKS — one `lock_guard` at the top of `RunRetentionSweep` makes the not-ready path's `LOG_WARN` re-enter the non-recursive `s_mutex`; the negative control dies with a fastfail. Order is copy → release → check → latch → sweep. New target `sein_retention_test` (14 checks) gives `PruneAgedLogs`/`PruneStaleProcessFolders` their first tests ever, behind a four-condition safety gate — they hold the DLL's only `remove_all`. | M / med |
...
| **AB20** ✅ | INFO | `Heiter.cpp:278` (g_hAutoStartThread) | **CLOSED 2026-08-19 — DUPLICATE of AB8, already fixed; no code change.** Same file, same defect, filed twice by different lenses. AB8's fix (L2) removed the file-static `g_hAutoStartThread`, closed the handle immediately so the thread runs detached, and corrected the two false comments. `Heiter.cpp:45-50` now states the corrected fact outright ("The two comments that used to claim a shutdown join were wrong") and `grep g_hAutoStartThread` returns only that note plus the back-reference at `:352`. Nothing left to fix. — *As filed:* Two comments claim shutdown waits for the auto-start thread via a stored handle; no code anywhere waits on it, and the handle is file-static so UE5_Shutdown cannot reach it | S / low |
| **AB21** ✅ | INFO | `Methode.cpp:352` (CEPlugin_InitializePlugin / CEPlugin_GetVersion / Ex) | **RE-VERIFIED 2026-08-19 — NEGATIVE RESULT STILL HOLDS, no code change.** Re-checked because `Methode.cpp` was touched the same day (`ab0fd6a6`, audit L2). Exactly three exports, all `__stdcall`, all matching `docs/ce-plugin-api-reference.md:60-70` on return type, parameter types and arity: `CEPlugin_GetVersion(CePluginVersion*, int)` `:423`, `CEPlugin_InitializePlugin(ExportedFunctions*, int)` `:444`, `CEPlugin_DisablePlugin(void)` `:478`. `CESDK_VERSION = 6` matches the SDK ceiling. Both documented traps honoured: the second `GetVersion` parameter is an int VALUE and is never dereferenced, and `pluginname` points at static storage. `sizeofExportedFunctions` is validated before the struct copy, and six `static_assert(offsetof(...))` guards make a layout drift a build failure. — *As filed:* Negative result — the static ABI surface itself is correct; no Radar/Radar.h finding from this lens | S / low |
| **AB22** ✅ | INFO | `Radar.h:80` (DataType (enum banner comment)) | **FIXED build 3268 (docs).** Direction: the COMMENT. The file header 28 lines above already recorded TArray<T> as shipped in build 757 (TSet/TMap in 927) and `Container` is a `DataType` the walker handles element-wise; what remains deferred is TOptional (V1c), which the header says in the same breath — so the deferral outlived the feature it referred to. Also dropped the pointer to "memory project_value_search_caveats", an assistant memory file git does not carry (working-lessons §7.1), in favour of the two specs in `docs/`. — *As filed:* The DataType enum's banner says TArray<T> scan "remains deferred" 28 lines after the file header says it shipped in build 757 | S / low |
| **AB23** | INFO | `Radar.h:676` (Radar::GroupSlotMatch::ownerClass) | **⬜ STILL OPEN — premise CONFIRMED, refactor deliberately deferred (2026-08-19).** The by-value `std::string ownerClass` per leaf is real and the accounting around it was worse than the finding said: `kMaxGroupSessionLeaves`'s own justification read "~120 B per GroupSlotMatch, so 4,000,000 leaves is roughly half a GB", which counts the string OBJECT and not the heap block it points at — UE class names routinely exceed the SSO buffer, so a worst-case session adds one allocation PER LEAF that the budget never sees. **Fixed the accounting** (the size is now derived as `kGroupSlotMatchBytes = sizeof(GroupSlotMatch)` so it cannot go stale, the under-count is documented, and a `static_assert` guards the PREMISE — the budget must stay under 1 GB of leaf records, which trips if the struct grows). **Did NOT intern**: the producers are in `Aura.cpp` and the reader in `Fern.cpp`, and **no test target compiles either**, so a producer/consumer refactor of the group-scan hot record could only be verified in-game — a bad trade on an INFO row during an unattended batch. Tracked in todo.md. — *As filed:* `GroupSlotMatch` carries a by-value `std::string` per leaf, re-introducing exactly the per-record heap string that V3-A's interning was built to remove | S / low |
...
| **AA35** ✅ | INFO | `ue5_freeze_helper.lua:355` (fetchInstancePage) | **FIXED build 3268.** Confirmed, and the claim is refuted against CE's own source rather than its docs: `readSmallIntegerEx` (`LuaHandler.pas:1614`) sets `signed:=false` when called with one argument and pushes `word(v)`, so the range is 0..65535 and `if returned < 0` was unreachable. `ue5_invoke_helper.lua:695-707` already documented the same fact correctly and passes `true` explicitly for signed reads (AA20) — two files in this repo disagreeing about one CE API was the part worth removing, not just the dead line. Also aligned the test rig: `freeze_helper_test.lua`'s stub was `readSmallInteger(a) return MEM[a]`, which would have let a negative fixture value through and made the dead branch look reachable; it now models CE's unsigned `word()` semantics like `invoke_helper_test.lua:93`. Lua rigs: 83 / 154 / 91, 0 failures. — *As filed:* The uint16 sign fixup is dead code resting on a false claim about CE's readSmallInteger | S / low |
| **AA36** ✅ | INFO | `ue5_invoke_helper.lua:88` (file-scope mailbox layout constants) | **FIXED build 3268 — the CI gate now READS the Lua side.** Confirmed as filed: `check_mailbox_contract.py` hashed `Mimic.h` and checked `CeMailboxLayout.ContractVersion`, and could not see **five** further hand-copies — `ue5_invoke_helper.lua`, `ue5_freeze_helper.lua`, the shipped `UE5CEDumper.CT`, and both offline Lua rigs. A gate with a hole reads as covered, which is worse than a known-absent gate. `check_lua_mirrors` now compares **67 literals across 5 files** against Mimic.h: offsets are **COMPUTED** from the packed struct (`#pragma pack(1)`) rather than read from the `// 0xNNN:` comments — which validates those comments too, since the Lua copies were made from them — `CMD_*`/`STATUS_*`/`INIT_*`/`LI_*` are compared to the same-named enum member, `ENTRY_SIZE_*`/`MAX_PAGES` to their `constexpr` originals, and each mirror's `UE5_SCRIPT_CONTRACT` must fall inside [MIN, CONTRACT]. The registry is **CLOSED**: an unregistered layout-shaped constant in a mirrored file is a hard failure, so the gap cannot reopen. **Contract did NOT move** (v3 / min 1) — this widens what the gate reads, nothing else. **Proven by 7 negative controls**, each mutated → checker red naming the constant → restored → green: an offset drift, a `Cmd` drift, an `InitState` drift in the .CT, a NEW unregistered constant, an out-of-range `UE5_SCRIPT_CONTRACT`, page-geometry drift, and a stale declared-offset comment in Mimic.h itself. — *As filed:* The CI contract gate hashes Mimic.h and CeMailboxLayout.cs only — the two standalone helpers hold a third hand-copied layout it cannot see | S / low |
...
| **Z17** ✅ | INFO | `LoggingService.cs:18` + `ILoggingService.cs:46` | **FIXED build 3268 (docs).** Direction: the DOCS, as the finding says — the code has always followed the age rule. `ILoggingService.cs:46`'s "2-version rotation" is replaced with the age-based description. ⚠ **Sibling-grep found two more stale numbers in the same `LoggingService.cs` header block, neither in the finding:** "Each file: 5MB cap" against `Constants.LogMaxSizeBytes = 8 * 1024 * 1024`, and "keep 15 days" against `Constants.LogMaxAgeDays = 21`. Both corrected, and both now NAME the constant instead of restating its value, so the next change cannot silently invalidate the prose again. — *As filed:* Two doc comments on the logging surface state a **generation-count** rotation policy ("2-version rotation") — the exact policy CLAUDE.md's app-data rule replaced with age-based retention, and which it explains at length *cannot* express the requirement. The code follows the age rule; only the docs are stale. Per this audit's own §1.11 lesson a comment admitting/asserting the wrong contract is worth filing, but there is no behavioural defect. | S / low |
...
**Current register: 3 open of 297 · 0 HIGH · 0 MED · 2 LOW · 1 INFO.** Re-derive with
