...
### ▶ THE NEXT SESSION STARTS HERE — the fix queue is down to ONE finding

> ## ✅ THE MED TIER IS EFFECTIVELY CLEAR — 19 → 1, and the one left was created today
>
> The 2026-08-17 session cleared the entire ORIGINAL MED tier (19 → 0), then a second pass the
> same day cleared the three it had spawned: **AA38** b3245 · **F9** b3247 · **A11** b3253.
>
> **The only MED open is [A12](#), split out of A11**: the GROUP-scan refine has the same
> stale-address defect. It is the cheapest MED this register has ever carried — the RULE is
> already written, pure, constexpr and pinned by 15 assertions plus two negative controls
> (`Radar::RefineContainerAnchor`). **All that is missing is the wiring**, and the row states the
> one trap: group candidates never pass through `emitCandidate`, so a required parameter there
> forces nothing. There are three hand-written metadata hops and each must be forced on its own.
>
> **AA39** (LOW, out of AA38) is the honest residual of the Pass-2 gate: Pass 1 is main-module-only
> and ungated by design. Its row says why that is smaller than it sounds and why the obvious
> tightening is wrong.

> ## ⚠ WHAT THE THREE 2026-08-17 PM FIXES TAUGHT — this is the reusable part
>
> All three findings had been marked "already re-derived and refute-passed" by the session that
> filed them. **All three were still wrong in ways that would have shipped a green commit with the
> defect intact**, and a fresh re-derivation + a three-lens adversarial pass caught every one. The
> pattern is not "the filer was careless" — it is that a finding filed *while fixing something else*
> inherits the parent's framing.
>
> - **A11 — the PREMISE was wrong in three places, and the prescribed fix fixed nothing.** It was
>   filed as deep/TSet-specific and A4-caused; it is neither (the defect dates to build **927** and
>   the code said so in a comment). Its stated mechanism (rehash relocates elements) is false in the
>   engine source. And the half it dismissed — *"for TArray that is mostly fine"* — is where the
>   **silent wrong value** lives. **Read a finding's dismissals as carefully as its claims.**
> - **F9 — the fix's own "keep this part verbatim" hid a second copy of the defect.** The component
>   lookup 45 lines below had the identical shape (`OwnedComponents` is a non-reflected **TSet**
>   asked for as an `ArrayProperty`). It was invisible because the outer bug meant **that loop had
>   never executed in production**. ⚠ **Never-run code is not known-good code** — and "it takes an
>   actor address and nothing else" is exactly the reasoning that let it through.
> - **AA38 — the filed fix was a MEASURED no-op.** It prescribed tightening an arm that
>   `TryResolveMatch` already makes unreachable for the winning pattern. The defect was real; the
>   location was not. **Check that the line you are about to change is on the path the repro takes.**
> - **Two boolean parameters that cannot both be true is an ENUM.** AA38's first draft had an
>   `anchorResolved` + `anchorIsMainExe` pair whose doc had to say *"meaningless when the first is
>   false, pass false here"* — reproducing, inside the fix, the exact trap the fix cited as its
>   design principle. Three states, 12 legal rows, no unreachable cells.
> - **Put the `static_assert` on a DIFFERENT cell from the one the negative control reverts.**
>   Otherwise the control becomes a compile abort and structurally cannot show its own
>   "everything else stayed green" half, which is the actual discrimination evidence.
> - **A negative control should also be able to fail the NON-regression.** A11's control 2 does not
>   delete a line — it substitutes the *plausible* rule (`nowNum != numAtScan`) and confirms three
>   assertions fail. Without it the suite would have certified a fix that dropped every candidate in
>   any array that merely grew.
...
**Current register: 167 open of 297 · 0 HIGH · 1 MED · 139 LOW · 27 INFO.** Re-derive with
