...
| [docs/todo.md](docs/todo.md) | **What's next** — open work only, with effort/risk tags: next-up, carryover gaps, speculative. Its `## Pending live-game verification` register owns everything shipped but unproven on a real game. |
| [docs/pending-verification_zh-TW.md](docs/pending-verification_zh-TW.md) | 🇹🇼 繁中 operational checklist — steps for the verification items **only a human can close** (act in-game, judge by eye, or no sample exists). Anything Auto + Computer Use can run belongs in todo.md. |
| [docs/log-verification-checklist.md](docs/log-verification-checklist.md) | **How to sweep a real session's logs** — the procedure half of todo.md's verification register: which file holds which marker, what to grep, what to do in-game first, and which absences prove nothing. |
