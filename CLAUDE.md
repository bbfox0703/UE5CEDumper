...
| [docs/archive/](docs/archive/) | Superseded docs, the older `dev-log` halves and closed `todo.md` sections, kept byte-identical **except where a row says otherwise** (two files were edited at archive time). Its [README](docs/archive/README.md) says what each file holds, which build ranges, and why it moved. |
