...
| `Arr_MulticastDelegates` | **2 elements; [0] EMPTY, [1] BOUND** to `D4b_OnPingProbe` | ⭐ **D3 / D3b — the only `TArray<multicast delegate>` here** (added 2026-09-08; [1] bound 2026-09-09). Two rows ride on it. **D3**: an element whose inner `TArray<FScriptDelegate>` header cannot be **read** must say `???`, not the affirmative `(0 bindings)` — three-state, rig `tools/verify/d3_delegate_array_unread.py`. **D3b**: the element STRIDE. `sizeof(FMulticastScriptDelegate)` is 16 in Shipping and **24** in a checked build, and the reader hardcoded 16. ⛔ Both elements were empty until 2026-09-09, which made that **unfalsifiable** — a right-stride and a wrong-stride read of zeros are the same string. With [0] empty and [1] bound, a 16-byte stride reads [1] from inside [0] and reports both empty. Rig: `tools/verify/d4b_delegate_pad.py` |
| `Arr_Delegates` | **2 elements; [0] EMPTY, [1] BOUND** to `D4b_OnPingProbe` | ⭐ **The SIXTH D4b site** (added 2026-09-09). The only `TArray<FScriptDelegate>` here — `Ubel::ReadDelegateArrayElements` had **no host at all**, and carried all three defects this file was swept for at once: an IGNORED `elemSize` parameter, a locally computed `8 + sizeof(FName)` stride, and both `Macht::ReadSafe` returns dropped. ⚠ An array's inner `FDelegateProperty` stores the STANDALONE `TScriptDelegate<FNotThreadSafeDelegateMode>`, which DOES carry the access detector — unlike a multicast's invocation-list elements. So the element stride is 24 on a checked build, not 16. `[0]` empty / `[1]` bound for the same reason as the row above |
| `OnActorHit` *(inherited)* | **BOUND** to `D4_OnActorHitProbe` in `BeginPlay` | ⭐ **D4 — the only BOUND `MulticastSparseDelegateProperty` here** (added 2026-09-08). All 16 sparse delegates on this actor previously read `(sparse, unbound)`, so `bIsBound == 0`, Ubel rejected before `Aura::WalkSparseDelegateBindings` was ever called, and the state the walker gets wrong — **located, but its InvocationList unreadable**, which used to render as the affirmative `(0 bindings, sparse)` — was unreachable on any host. The handler is deliberately EMPTY: an actor that reacted to being hit would change every other row that shares this fixture |
| `Multicast_Inline` | **BOUND** to `D4b_OnPingProbe` | ⭐ **D4b — the only non-array `MulticastInlineDelegateProperty` here** (added 2026-09-09). Ubel's single-field multicast reader had no host on this fixture at all, so it had never met a build where the `InvocationList` starts at +8 instead of +0. Bound deliberately: an unbound one reads `(0 bindings)` at either offset and proves nothing |
| `Del_Unicast` | **BOUND** to `D4b_OnPingProbe` | ⭐ **D4b — the only `DelegateProperty` here** (added 2026-09-09). ⚠ A standalone `FScriptDelegate` is `TScriptDelegate<FNotThreadSafeDelegateMode>` and **is** padded by the access detector, unlike a multicast's invocation-list elements, which are the `...NotChecked...` variant and never are. Our comments call both "FScriptDelegate"; this row is the host that tells them apart. Expect `DumperTestActor_*::D4b_OnPingProbe`, not `(unbound)` |
