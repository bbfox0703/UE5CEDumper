...
def main() -> int:
    ...
    fails = arm(game_tid, 0.0, D1_CAUSE,
                "ARM 1 (D1): freeze BELOW the stall threshold -- responsive, refused anyway")
    ...
    if fails:
        ...
        for f in fails:
            print("  - %s" % f)
        return 1
    print("D1: PASS -- a dispatcher refusal (arm 1) and an unresponsive thread (arm 2) are "
          "reported as DIFFERENT causes, both keep the record, and in both the restore lands "
          "on its own once the game thread comes back.")
    return 0
